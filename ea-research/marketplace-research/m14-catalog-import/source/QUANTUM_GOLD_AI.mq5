...
//|                      QUANTUM GOLD AI                              |
//|                  by  ALGOTRADERS24 AI                             |
//|         Single-Asset Quantum Engine - XAUUSD (GOLD)              |
//|                                                                   |
//|  OK: Works on ALL brokers - auto prefix/suffix detection          |
//|  OK: Gold-tuned defaults (ATR-based volatility handling)          |
...
#property description "QUANTUM GOLD AI - Single-Asset Gold Trading System by ALGOTRADERS24 AI"
#property strict
...
#define EA_NAME    "QUANTUM GOLD AI"
#define EA_COMPANY "ALGOTRADERS24 AI"
#define EA_ASSET   "GOLD"
...
input group "=== ASSET ==="
input string Symbol_Override = "";       // Leave blank = trade the chart symbol. Or force one (e.g. XAUUSD.m)
...
input group "=== SL-TP MODE (Gold-tuned) ==="
input bool   Use_Fixed_SLTP   = false;   // true=Fixed Points, false=ATR Dynamic
input int    Fixed_SL_Points  = 2000;    // Fixed SL points (gold: ~$2.00 on 3-digit)
input int    Fixed_TP_Points  = 3500;    // Fixed TP points
input double ATR_SL_Mult      = 2.0;     // Gold ATR x SL
input double ATR_TP_Mult      = 3.2;     // Gold ATR x TP
...
input int    News_MinsBefore  = 30;      // Stop trading X minutes BEFORE news
input int    News_MinsAfter   = 30;      // Resume X minutes AFTER news
input bool   News_AllCurrencies = true;  // Watch ALL currencies (else USD/EUR/GBP below)
...
input int    Magic_Num   = 24010001;     // Unique per EA
input string EA_Tag      = "QGOLD_AI";
...
string pfx = "QGOLD_";
color  CLR_ACCENT  = C'255,190,40';   // bright gold accent
color  CLR_ACCENT2 = C'255,150,30';   // amber secondary
color  CLR_BG      = C'16,13,8';      // deep gold-black
color  CLR_PANEL   = C'24,19,10';     // dark amber panel
...
int OnInit()
{
   ...
   // M15 confirmation handles (fast EMA, slow EMA, RSI on Confirm_TF)
   ...
   EventSetTimer(1);
   BuildDashboard();
   Print("=== ",EA_NAME," v1.36 by ",EA_COMPANY," READY | Symbol: ",gSym," | ",
         (Use_Fixed_SLTP?"FIXED":"ATR")," SL | M15: ",(Use_M15_Confirm?"ON":"OFF")," ===");
   return INIT_SUCCEEDED;
}
...
void OnTick()
{
   // Refresh cached news/daily status every 15s (keeps dashboard light).
   ...
   if(cb!=0 && gLastBar!=0 && cb!=gLastBar){
      ...
      bool newsOK=!IsNewsTime();                                        // pause around news
      bool dayOK =(Max_Trades_Day<=0 || TradesToday(gSym)<Max_Trades_Day); // per-symbol daily cap
      if(!gUserPause&&!gDDPause&&sessOK&&friOK&&newsOK&&dayOK&&!HasPos() && q!=DATA_NR){
         ...
         int cf=MConfirm();   // M15 confirmation gate (auto entries only)
...
// M15 confirmation: +1 = BUY agrees, -1 = SELL agrees, 0 = no confirm.
// Uses fast EMA, slow EMA and RSI on Confirm_TF (same periods as the engine).
int gCF_Status=0;   // last computed confirm state (for dashboard)
int MConfirm()
{
...
// News filter via MT5 built-in Economic Calendar. Auto-disabled in tester.
bool IsNewsTime()
{
...
// Count NEW trades opened today for a symbol (this EA only). Resets daily.
...
void DrawLogo(int x,int y)
{
   ...
   oRect("lg_core",x+21,y+21,12,12,CLR_ACCENT,CLR_ACCENT);
...
// Segmented signal meter (robotic HUD)
...
{
   ObjectsDeleteAll(0,pfx);
   int X=Dashboard_X,Y=Dashboard_Y,W=392,H=438;

   // layered shadow
   oRect("shadow2",X+8,Y+8,W,H,C'4,3,1',C'4,3,1');
   oRect("shadow",X+4,Y+4,W,H,C'8,6,3',C'8,6,3');
   ...
   // Header with AI core logo
   oRect("hdr",X,Y+3,W,66,C'28,22,10',CLR_ACCENT,1);
   DrawLogo(X+10,Y+10);
   oText("co", X+76,Y+11, EA_COMPANY,CLR_ACCENT2,8,"Consolas",true);
   oText("nm", X+76,Y+26,EA_NAME,CLR_ACCENT,13,"Consolas",true);
   oText("tag",X+76,Y+48,"GOLD AI ENGINE . XAUUSD",C'150,120,60',7);
   oText("core",X+W-92,Y+11,"CORE STATUS",C'150,120,60',7);
   oText("corev",X+W-92,Y+24,"* ONLINE",C'0,255,157',9,"Consolas",true);

   int sy=Y+80;

   // Data grid 2x2 (HUD)
   oRect("dg",X+10,sy,W-20,52,CLR_PANEL,C'60,46,18',1);
   oText("balL",X+18,sy+5,"BALANCE",C'150,120,60',7);
   oText("bal", X+18,sy+16,"",C'255,220,150',10,"Consolas",true);
   oText("eqL", X+205,sy+5,"EQUITY",C'150,120,60',7);
   oText("eq",  X+205,sy+16,"",C'255,220,150',10,"Consolas",true);
   oText("pnlL",X+18,sy+30,"OPEN P&L",C'150,120,60',7);
   oText("pnl", X+18,sy+40,"",C'200,200,200',10,"Consolas",true);
   oText("ddL", X+205,sy+30,"DRAWDOWN",C'150,120,60',7);
   oText("dd",  X+205,sy+40,"",C'200,200,200',10,"Consolas",true);
   sy+=60;

   // Performance stats (3 boxes) - REAL data
   int bw=(W-20-16)/3;
   oRect("st1",X+10,      sy, bw,38,CLR_PANEL,C'60,46,18',1);
   oRect("st2",X+10+bw+8, sy, bw,38,CLR_PANEL,C'60,46,18',1);
   oRect("st3",X+10+2*(bw+8),sy,bw,38,CLR_PANEL,C'60,46,18',1);
   oText("st1v",X+18,        sy+5,"",CLR_ACCENT,12,"Consolas",true);
   oText("st1l",X+18,        sy+24,"TRADES",C'150,120,60',7);
   oText("st2v",X+18+bw+8,   sy+5,"",CLR_ACCENT,12,"Consolas",true);
   oText("st2l",X+18+bw+8,   sy+24,"WIN RATE",C'150,120,60',7);
   oText("st3v",X+18+2*(bw+8),sy+5,"",CLR_ACCENT,12,"Consolas",true);
   oText("st3l",X+18+2*(bw+8),sy+24,"NET PROFIT",C'150,120,60',7);
   sy+=46;

   // Signal analysis
   oText("siglbl",X+14,sy,"> NEURAL SIGNAL ANALYSIS",C'150,120,60',8,"Consolas",true);
   oText("sym",  X+250,sy,"",CLR_ACCENT,9,"Consolas",true); sy+=18;
   oText("sigbar",X+14,sy,"□□□□□□□□□□",C'70,56,24',15,"Consolas",true); sy+=24;
   oText("sigdir",X+14,sy,"",C'120,120,120',13,"Consolas",true);
   oText("sigpct",X+300,sy,"",C'120,120,120',15,"Consolas",true);
   oText("sigpos",X+14,sy+20,"",C'150,130,90',7); sy+=34;

   // Command buttons
   oBtn("b_buy", X+10, sy,118,30,"^ BUY",C'0,184,126',C'2,22,15',11);
   oBtn("b_sell",X+136,sy,118,30,"v SELL",C'208,69,69',C'255,255,255',11);
   oBtn("b_close",X+262,sy,120,30,"x CLOSE",C'200,150,32',C'26,18,0',10);
   sy+=37;

   oBtn("b_pause", X+10, sy,184,26,"|| PAUSE",C'44,38,19',C'200,170,90',9);
   oBtn("b_resume",X+200,sy,182,26,"> RESUME",C'19,44,30',C'0,255,157',9);
   sy+=32;

   // Footer
   oRect("ftr",X,Y+H-26,W,26,C'14,11,6',CLR_ACCENT,1);
...
void UpdateDashboard()
{
   ...
   sC("dd",(dd<8)?C'255,190,40':(dd<16)?C'255,150,30':C'220,80,80');
   ...
   if(gProb==DATA_NR){
      sT("sigbar","loading data..."); sC("sigbar",C'120,100,50');
      sT("sigdir","..."); sC("sigdir",C'150,130,80'); sT("sigpct","");
   } else {
      double p=gProb;
      double thr=Signal_Threshold/100.0;
      ...
      // M15 confirmation indicator: agree (checkmark) / disagree
      if(Use_M15_Confirm){
         int cf=MConfirm();
      ...
      double relThr=Signal_Threshold/100.0; if(relThr<0.05)relThr=0.05;
      double relPct=MathMin(MathAbs(p)/relThr,1.0)*100.0;
      sT("sigpct",DoubleToString(relPct,0)+"%"); sC("sigpct",dc);
   }
   ...
   } else { sT("sigpos","no position"); sC("sigpos",C'150,130,90'); }
   ...
   else { sBg("b_pause",C'44,38,19');
      string rs=(Auto_RiskScale && gLossStreak>=Scale_LossStreak)?" | RISK-SCALED":"";
      sT("status","> GOLD AI ACTIVE | "+gSym+" | "+(Use_Fixed_SLTP?"FIXED":"ATR")+" SL | THR "+DoubleToString(Signal_Threshold,0)+"% | M15:"+(Use_M15_Confirm?"ON":"OFF")+rs);
   }
   ChartRedraw(0);
}
