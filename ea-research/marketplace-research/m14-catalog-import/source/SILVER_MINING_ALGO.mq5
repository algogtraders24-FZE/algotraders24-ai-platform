...
//|                      SILVER MINING ALGO                              |
//|                  by  ALGOTRADERS24 AI                             |
//|         Single-Asset Quantum Engine - SILVER              |
//|                                                                   |
//|  OK: Works on ALL brokers - auto prefix/suffix detection          |
//|  OK: SILVER-tuned defaults (ATR-based volatility handling)          |
...
#property description "SILVER MINING ALGO - Single-Asset Silver Trading System by ALGOTRADERS24 AI"
#property strict
...
#define EA_NAME    "SILVER MINING ALGO"
#define EA_COMPANY "ALGOTRADERS24 AI"
#define EA_ASSET   "SILVER"
...
input group "=== ASSET ==="
input string Symbol_Override = "";       // Leave blank = trade the chart symbol. Or force one (e.g. XAGUSD.m)
...
input group "=== SL-TP MODE (Silver-tuned) ==="
input bool   Use_Fixed_SLTP   = false;   // true=Fixed Points, false=ATR Dynamic
input int    Fixed_SL_Points  = 1500;    // Fixed SL points (silver-tuned)
input int    Fixed_TP_Points  = 3000;    // Fixed TP points
input double ATR_SL_Mult      = 2.5;     // SILVER ATR x SL
input double ATR_TP_Mult      = 4.0;     // SILVER ATR x TP
...
input int    Fixed_Trail_Pts  = 1200;
input bool   Use_BreakEven    = true;
input double MaxDrawdown_Pct  = 28.0;
...
input double Strength_Min = 20.0;     // Minimum trend strength
...
input int    Magic_Num   = 24010002;     // Unique per EA
input string EA_Tag      = "SILVER_MINING";
...
string pfx = "QSILVER_";
color  CLR_ACCENT  = C'220,225,235';   // bright silver/platinum accent
color  CLR_ACCENT2 = C'130,180,210';   // cool steel-blue
color  CLR_BG      = C'14,18,26';   // cool steel dark
color  CLR_PANEL   = C'20,26,36';   // steel panel
...
// Brand mark: drawn from primitives (no external file - Market-listing safe)
void DrawLogo(int x,int y)
{
   oRect("lg_hex",x,y,52,52,CLR_PANEL,CLR_ACCENT,2);
   // neural-net style dots row
   oRect("lg_d1",x+8, y+8, 4,4,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_d2",x+24,y+6, 4,4,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_d3",x+40,y+8, 4,4,CLR_ACCENT2,CLR_ACCENT2);
   // candlesticks
   oRect("lg_c1",x+12,y+18,5,24,C'0,224,176',C'0,224,176');
   oRect("lg_c2",x+24,y+12,5,32,C'10,163,255',C'10,163,255');
   oRect("lg_c3",x+36,y+24,5,20,C'255,90,106',C'255,90,106');
}
...
{
   ObjectsDeleteAll(0,pfx);
   int X=Dashboard_X,Y=Dashboard_Y,W=380,H=346;

   oRect("shadow",X+5,Y+5,W,H,C'3,6,10',C'3,6,10');
   oRect("bg",X,Y,W,H,CLR_BG,CLR_ACCENT,2);

   // Header band with embedded brand logo
   oRect("hdr",X,Y,W,64,C'24,30,42',CLR_ACCENT,1);
   DrawLogo(X+8,Y+4);                       // 64x64 logo image
   oText("co", X+80,Y+8, EA_COMPANY,CLR_ACCENT2,9,"Consolas",true);
   oText("nm", X+80,Y+26,EA_NAME,CLR_ACCENT,13,"Consolas",true);
   oText("tag",X+80,Y+48,"Single-Asset Quantum Engine",C'120,140,150',7);

   // Account row
   int sy=Y+72;
   oText("bal",X+12,sy,"",CLR_ACCENT2,9,"Consolas",true);
   oText("eq", X+200,sy,"",CLR_ACCENT2,9,"Consolas",true); sy+=17;
   oText("pnl",X+12,sy,"",C'200,200,200',9,"Consolas",true);
   oText("dd", X+200,sy,"",C'200,200,200',9,"Consolas",true); sy+=20;

   // Big signal box
   oRect("sigbox",X+12,sy,W-24,70,CLR_PANEL,C'30,60,55',1);
   oText("sym",  X+22,sy+8, "",CLR_ACCENT,11,"Consolas",true);
   oText("slm",  X+240,sy+8,"",C'255,200,0',8);
   oText("sigbar",X+22,sy+30,"░░░░░░░░░░░░░░",C'0,100,80',12,"Consolas",true);
   oText("sigdir",X+22,sy+50,"",C'120,120,120',12,"Consolas",true);
   oText("sigpos",X+200,sy+50,"",C'160,160,160',8);
   sy+=80;

   // Manual buttons
   oBtn("b_buy", X+12, sy,108,28,"BUY",C'0,170,110',C'255,255,255',11);
   oBtn("b_sell",X+126,sy,108,28,"SELL",C'200,55,55',C'255,255,255',11);
   oBtn("b_close",X+240,sy,108,28,"CLOSE",C'190,130,0',C'0,0,0',10);
   sy+=34;

   oBtn("b_pause", X+12, sy,168,24,"PAUSE",C'40,40,80',C'180,180,255',9);
   oBtn("b_resume",X+186,sy,162,24,"RESUME",C'0,60,40',C'0,255,160',9);
   sy+=30;

   // Status footer
   oRect("ftr",X,Y+H-24,W,24,C'8,20,17',CLR_ACCENT,1);
   oText("status",X+12,Y+H-18,"",CLR_ACCENT2,7);
...
void UpdateDashboard()
{
   ...
   if(gProb==DATA_NR){
      ...
      string m15="";
      if(Use_M15_Confirm){ int cf=MConfirm(); bool ag=((p>thr&&cf==1)||(p<-thr&&cf==-1)); m15=ag?"  M15+":"  M15x"; }
