...
//|                      QuantumPulse BTC                              |
//|                  by  ALGOTRADERS24 AI                             |
//|         Single-Asset Quantum Engine - BTC              |
//|                                                                   |
//|  OK: Works on ALL brokers - auto prefix/suffix detection          |
//|  OK: BTC-tuned defaults (ATR-based volatility handling)          |
...
#property description "QuantumPulse BTC - Single-Asset Btc Trading System by ALGOTRADERS24 AI"
#property strict
...
#define EA_NAME    "QuantumPulse BTC"
...
input group "=== ASSET ==="
input string Symbol_Override = "";       // Leave blank = trade the chart symbol. Or force one (e.g. BTCUSD.m)
...
input group "=== SL-TP MODE (Btc-tuned) ==="
input bool   Use_Fixed_SLTP   = false;   // true=Fixed Points, false=ATR Dynamic
input int    Fixed_SL_Points  = 30000;   // Fixed SL points (BTC-tuned)
input int    Fixed_TP_Points  = 60000;    // Fixed TP points
input double ATR_SL_Mult      = 3.0;     // BTC ATR x SL
input double ATR_TP_Mult      = 5.0;     // BTC ATR x TP
...
input int    Fixed_Trail_Pts  = 20000;
input bool   Use_BreakEven    = true;
input double MaxDrawdown_Pct  = 35.0;
...
input int    Magic_Num   = 24010004;     // Unique per EA
input string EA_Tag      = "QPULSE_BTC";
...
string pfx = "QBTC_";
color  CLR_ACCENT  = C'247,170,40';   // bitcoin gold accent
color  CLR_ACCENT2 = C'255,215,120';   // bright amber
color  CLR_BG      = C'10,16,24';
color  CLR_PANEL   = C'13,23,34';
...
int OnInit()
{
   ...
   EventSetTimer(1);
   BuildDashboard();
   Print("=== ",EA_NAME," v1.20 by ",EA_COMPANY," READY | Symbol: ",gSym," | ",
...
// Brand mark: Bitcoin-style sphere with neural nodes + pulse (drawn, Market-safe)
void DrawLogo(int x,int y)
{
   // outer ring (sphere)
   oRect("lg_ring",x,y,54,54,CLR_PANEL,CLR_ACCENT,2);
   // inner core
   oRect("lg_core",x+22,y+22,10,10,CLR_ACCENT,CLR_ACCENT);
   // neural nodes around core
   oRect("lg_n1",x+10,y+10,5,5,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_n2",x+39,y+10,5,5,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_n3",x+10,y+39,5,5,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_n4",x+39,y+39,5,5,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_n5",x+25,y+6, 4,4,CLR_ACCENT2,CLR_ACCENT2);
   oRect("lg_n6",x+25,y+44,4,4,CLR_ACCENT2,CLR_ACCENT2);
   // pulse line through center
   oRect("lg_p1",x+4, y+27,8,2,CLR_ACCENT,CLR_ACCENT);
   oRect("lg_p2",x+14,y+24,3,8,CLR_ACCENT,CLR_ACCENT);
   oRect("lg_p3",x+18,y+20,3,16,CLR_ACCENT,CLR_ACCENT);
   oRect("lg_p4",x+37,y+27,13,2,CLR_ACCENT,CLR_ACCENT);
}

string Bar(double p){
   int n=(int)(MathAbs(p)*14); string b="";
   for(int i=0;i<14;i++) b+=(i<n?"█":"░");
   return b;
}
...
{
   ObjectsDeleteAll(0,pfx);
   int X=Dashboard_X,Y=Dashboard_Y,W=392,H=430;

   // layered shadow for depth
   oRect("shadow2",X+8,Y+8,W,H,C'2,4,7',C'2,4,7');
   oRect("shadow",X+4,Y+4,W,H,C'4,7,11',C'4,7,11');
   oRect("bg",X,Y,W,H,CLR_BG,CLR_ACCENT,2);

   // top accent strip
   oRect("accent",X,Y,W,3,CLR_ACCENT,CLR_ACCENT);

   // Header band with brand logo
   oRect("hdr",X,Y+3,W,66,C'32,24,8',CLR_ACCENT,1);
   DrawLogo(X+10,Y+10);
   oText("co", X+76,Y+11, EA_COMPANY,CLR_ACCENT2,9,"Consolas",true);
   oText("nm", X+76,Y+28,EA_NAME,CLR_ACCENT,14,"Consolas",true);
   oText("tag",X+76,Y+50,"Intelligent Mining . Algorithmic Edge",C'150,140,110',7);

   // Account panel
   int sy=Y+78;
   oRect("accpanel",X+10,sy-4,W-20,42,CLR_PANEL,C'40,50,60',1);
   oText("bal",X+18,sy,"",CLR_ACCENT2,9,"Consolas",true);
   oText("eq", X+210,sy,"",CLR_ACCENT2,9,"Consolas",true); sy+=18;
   oText("pnl",X+18,sy,"",C'200,200,200',9,"Consolas",true);
   oText("dd", X+210,sy,"",C'200,200,200',9,"Consolas",true); sy+=28;

   // Performance stats row (3 boxes: trades, win rate, net profit) - REAL data
   int bw=(W-20-16)/3;   // 3 boxes with 8px gaps
   oRect("st1",X+10,      sy, bw,40,CLR_PANEL,C'60,50,20',1);
   oRect("st2",X+10+bw+8, sy, bw,40,CLR_PANEL,C'60,50,20',1);
   oRect("st3",X+10+2*(bw+8),sy,bw,40,CLR_PANEL,C'60,50,20',1);
   oText("st1v",X+18,        sy+6,"",CLR_ACCENT,13,"Consolas",true);
   oText("st1l",X+18,        sy+25,"TRADES",C'140,125,90',7);
   oText("st2v",X+18+bw+8,   sy+6,"",CLR_ACCENT,13,"Consolas",true);
   oText("st2l",X+18+bw+8,   sy+25,"WIN RATE",C'140,125,90',7);
   oText("st3v",X+18+2*(bw+8),sy+6,"",CLR_ACCENT,13,"Consolas",true);
   oText("st3l",X+18+2*(bw+8),sy+25,"NET PROFIT",C'140,125,90',7);
   sy+=48;

   // Big signal box
   oRect("sigbox",X+10,sy,W-20,76,CLR_PANEL,CLR_ACCENT,1);
   oText("sym",  X+20,sy+8, "",CLR_ACCENT,11,"Consolas",true);
   oText("slm",  X+250,sy+8,"",C'255,200,0',8);
   oText("sigbar",X+20,sy+32,"##############",C'80,70,30',13,"Consolas",true);
   oText("sigdir",X+20,sy+54,"",C'120,120,120',12,"Consolas",true);
   oText("sigpos",X+210,sy+54,"",C'160,160,160',8);
   sy+=86;

   // Manual buttons
   oBtn("b_buy", X+10, sy,118,30,"BUY",C'0,170,110',C'255,255,255',11);
   oBtn("b_sell",X+136,sy,118,30,"SELL",C'200,55,55',C'255,255,255',11);
   oBtn("b_close",X+262,sy,120,30,"CLOSE ALL",C'190,130,0',C'0,0,0',9);
   sy+=37;

   oBtn("b_pause", X+10, sy,184,26,"PAUSE",C'40,40,80',C'180,180,255',9);
   oBtn("b_resume",X+200,sy,182,26,"RESUME",C'0,60,40',C'0,255,160',9);
   sy+=32;

   // Status footer
   oRect("ftr",X,Y+H-26,W,26,C'8,20,17',CLR_ACCENT,1);
   oText("status",X+12,Y+H-19,"",CLR_ACCENT2,7);
...
void UpdateDashboard()
{
   ...
   sT("bal","Bal $"+DoubleToString(bal,2));
   sT("eq", "Eq $"+DoubleToString(eq,2));
   sT("pnl",(pnl>=0?"PnL +$":"PnL -$")+DoubleToString(MathAbs(pnl),2));
   sC("pnl",(pnl>=0)?C'0,220,120':C'220,80,80');
   sT("dd","DD "+DoubleToString(dd,1)+"%");
   sC("dd",(dd<8)?C'0,220,120':(dd<16)?C'255,200,0':C'220,80,80');
   sT("slm",(Use_Fixed_SLTP?"[FIXED SL]":"[ATR SL]"));
   ...
   if(!gReady){
      sT("sym",EA_ASSET+": NOT FOUND");
      sT("sigbar","──────────────"); sC("sigbar",C'90,90,90');
      sT("sigdir","CHECK SYMBOL"); sC("sigdir",C'220,80,80');
      sT("status","WARNING: "+EA_ASSET+" symbol not available - set Symbol_Override in inputs");
   ...
   if(gProb==DATA_NR){
      ...
      sT("sigdir","..."); sC("sigdir",C'150,150,80');
   } else {
      double p=gProb;
      double thr=Signal_Threshold/100.0;
      sT("sigbar",Bar(p)); sC("sigbar",(p>0)?C'0,220,120':C'220,70,70');
      double rt=thr; if(rt<0.05)rt=0.05;
      double relP=MathMin(MathAbs(p)/rt,1.0)*100.0;
      string m15=""; if(Use_M15_Confirm){ int cf=MConfirm(); bool ag=((p>thr&&cf==1)||(p<-thr&&cf==-1)); m15=ag?"  M15+":"  M15x"; }
      string d=(p>thr)?"▲ STRONG BUY  "+DoubleToString(relP,0)+"%"+m15:
               (p<-thr)?"▼ STRONG SELL  "+DoubleToString(relP,0)+"%"+m15:
               "· NEUTRAL  "+DoubleToString(relP,0)+"%";
      color dc=(p>thr)?C'0,255,120':(p<-thr)?C'255,80,80':C'150,150,150';
   ...
   } else { sT("sigpos","no position"); sC("sigpos",C'130,130,130'); }
   ...
   else { sBg("b_pause",C'40,40,80');
      sT("status","* LIVE | "+gSym+" | "+(Use_Fixed_SLTP?"Fixed":"ATR")+" SL | Thr "+DoubleToString(Signal_Threshold,0)+"% | M15:"+(Use_M15_Confirm?"ON":"OFF"));
   }
   ChartRedraw(0);
}
