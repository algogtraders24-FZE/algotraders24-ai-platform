...
//|                  by  ALGOTRADERS24 AI                             |
//|     Multi-Index Quantum Engine - 6 Major Indices       |
//|                                                                   |
//|  OK: Works on ALL brokers - auto prefix/suffix detection          |
//|  OK: Trades US30/NAS100/SPX500/GER40/UK100/JP225       |
...
#property description "QUANTUM INDEX ENGINE - Multi-Index Trading System by ALGOTRADERS24 AI"
#property strict
...
#define EA_NAME    "QUANTUM INDEX ENGINE"
#define EA_COMPANY "ALGOTRADERS24 AI"
#define EA_ASSET   "INDEX"
...
input group "=== ASSET ==="
input string Symbol_Override = "";       // Leave blank = trade the chart symbol. Or force one (e.g. USTEC, US30)
...
input bool   Trade_All_Pairs  = true;    // Trade every detected index (multi-index mode)
...
input int    Max_Open_Trades  = 6;       // Max simultaneous open trades (6 indices)
input double MaxDrawdown_Pct  = 30.0;    // Max drawdown % (indices are volatile)
...
input int    Magic_Num   = 24050001;     // Unique per EA
input string EA_Tag      = "QIDXENGINE";
...
input group "=== MULTI-INDEX ENGINE ==="
...
input bool   Auto_Detect    = true;     // Auto-find all 6 major indices at your broker
input string Custom_Pairs   = "";       // Optional: limit to some (e.g. US30,NAS100). Blank=all 6.
...
string pfx = "QIE_";
color  CLR_ACCENT  = C'255,170,40';   // amber (global markets)
color  CLR_ACCENT2 = C'0,200,255';    // cyan
color  CLR_BG      = C'14,12,8';       // deep warm-black
color  CLR_PANEL   = C'22,18,12';      // panel

// Apply tuning to the chart symbol.
// Uses substring matching so it works with ANY broker prefix/suffix, e.g.
// Works with any broker index naming: US30, US100, USTEC, GER40, etc.
void DetectAndTune()
{
...
void DetectPairs()
{
   ...
   if(gInTester) return;   // tester is single-symbol; skip multi-index entirely

   // Clean labels shown on dashboard
   string labels[6] = {"US30","NAS100","SPX500","GER40","UK100","JP225"};

   // If user gave a custom list, honor it (match against labels/aliases)
   ...
   for(int k=0;k<6;k++){
      // user limited the set? skip labels not requested
      if(onlyN>0){
         bool req=false;
         for(int j=0;j<onlyN;j++) if(StringFind(only[j],labels[k])>=0 || StringFind(labels[k],only[j])>=0) req=true;
         if(!req) continue;
      }

      string al[];
      if(labels[k]=="US30")        { string t[]={"US30","DJ30","WS30","DOW","WALL","US30Cash","DJIUSD","USTECDJ"}; ArrayCopy(al,t); }
      else if(labels[k]=="NAS100") { string t[]={"USTEC","US100","NAS100","NDX100","NASDAQ","TECH100","USTECH","NAS100Cash"}; ArrayCopy(al,t); }
      else if(labels[k]=="SPX500") { string t[]={"US500","SPX500","SP500","SPX","SP500Cash","US500Cash"}; ArrayCopy(al,t); }
      else if(labels[k]=="GER40")  { string t[]={"GER40","DE40","DAX40","DAX","GER30","DE30","GER40Cash"}; ArrayCopy(al,t); }
      else if(labels[k]=="UK100")  { string t[]={"UK100","FTSE100","FTSE","UK100Cash","GB100"}; ArrayCopy(al,t); }
      else                         { string t[]={"JP225","JPN225","NIKKEI","JP225Cash","JPN225Cash","N225"}; ArrayCopy(al,t); }

      string resolved;
      if(ResolveIndexAliases(al,resolved)){
         // avoid duplicates (e.g. an alias that already matched another index)
         bool dup=false; for(int d=0;d<gPairCount;d++) if(gScanName[d]==resolved) dup=true;
         if(!dup){
            gScanName[gPairCount]=resolved;
            gScanLabel[gPairCount]=labels[k];
            gScanProb[gPairCount]=DATA_NR;
            gPairCount++;
         }
      }
   }
   Print("QUANTUM INDEX ENGINE: detected ",gPairCount," tradable indices at this broker");
}
...
void OnTick()
{
   ...
   if(cb!=0 && gLastBar!=0 && cb!=gLastBar){
      ...
      // Multi-index auto-trading runs ONCE per new bar (not every tick),
      // so a fresh signal opens at most one new position per index per bar.
...
{
   if(label=="US30")        { slPts=4000; tpPts=7000; }   // ~44,000, big ranges
   else if(label=="NAS100") { slPts=3000; tpPts=5000; }   // ~20,000
   else if(label=="SPX500") { slPts=800;  tpPts=1400; }   // ~5,800
   else if(label=="GER40")  { slPts=2500; tpPts=4200; }   // ~18,000
   else if(label=="UK100")  { slPts=1200; tpPts=2000; }   // ~8,200
   else if(label=="JP225")  { slPts=3500; tpPts=6000; }   // ~40,000
   else                     { slPts=Fixed_SL_Points; tpPts=Fixed_TP_Points; }
}
...
void MultiExec(int idx,bool isBuy)
{
   ...
   double sd,td;
   if(Use_Fixed_SLTP){
      double slP,tpP; IndexFixedPts(gScanLabel[idx],slP,tpP);
      sd=slP*pt; td=tpP*pt;
   } else {
...
void UpdateDashboard()
{
   ...
   gCanvas.FontSet("Arial Bold",17);
   gCanvas.TextOut(14,10,"QUANTUM INDEX ENGINE",AX(CLR_ACCENT));
   gCanvas.FontSet("Arial",9);
   gCanvas.TextOut(16,33,"MULTI-INDEX NEURAL ENGINE",AX((color)C'180,160,120'));
   ...
   // ---- right panel: 6-index grid (single column, big rows) ----
   int rpx=162, rpy=56, rpw=W-rpx-6, rph=H-138;
   Panel(rpx,rpy,rpw,rph,AX((color)C'10,16,34',255),AX(CLR_ACCENT,160));
   gCanvas.FontSet("Arial Bold",10);
   gCanvas.TextOut(rpx+8,rpy+4,(Multi_Trade?"INDEX GRID [LIVE]  ":"INDEX SCANNER  ")+IntegerToString(gPairCount)+" INDICES",AX((color)C'190,210,240'));

   double thr=Signal_Threshold/100.0;
   int gridTop=rpy+22;
   int rows=(gPairCount>6)?6:gPairCount; if(rows<1) rows=6;
   int rowH=(rph-26)/((gPairCount>0)?gPairCount:6);
   for(int i=0;i<gPairCount && i<6;i++){
      int ry=gridTop+i*rowH;
      string nm=gScanLabel[i];
      double p=gScanProb[i];
      Panel(rpx+6,ry,rpw-12,rowH-4,AX((color)C'14,22,40',255),AX((color)C'30,50,80',180));
      gCanvas.FontSet("Arial Bold",12);
      gCanvas.TextOut(rpx+14,ry+5,nm,AX((color)C'220,230,245'));
      if(gScanName[i]=="" || p==DATA_NR){
         gCanvas.FontSet("Arial",9);
         gCanvas.TextOut(rpx+90,ry+6,"scanning...",AX((color)C'120,130,150'));
      } else {
         string dir=(p>thr)?"BUY":(p<-thr)?"SELL":"WAIT";
         uint dc=(p>thr)?AX((color)C'40,255,150'):(p<-thr)?AX((color)C'255,80,80'):AX((color)C'200,200,120');
         gCanvas.FontSet("Arial Bold",12);
         gCanvas.TextOut(rpx+96,ry+5,dir,dc);
         double rt=thr; if(rt<0.05)rt=0.05;
         double relP=MathMin(MathAbs(p)/rt,1.0)*100.0;
         string m15=""; if(Use_M15_Confirm){ bool ag=((p>thr&&gScanCF[i]==1)||(p<-thr&&gScanCF[i]==-1)); m15=ag?" M15+":" M15x"; }
         gCanvas.TextOut(rpx+150,ry+5,DoubleToString(relP,0)+"%"+m15,dc);
         DrawSpark(rpx+rpw-92,ry+5,74,rowH-16,i,dc);
         // strength bar
         gCanvas.FillRectangle(rpx+14,ry+rowH-11,rpx+rpw-20,ry+rowH-7,AX((color)C'30,40,60'));
         int pw=(int)((rpw-34)*MathMin(1.0,MathAbs(p)));
         gCanvas.FillRectangle(rpx+14,ry+rowH-11,rpx+14+pw,ry+rowH-7,dc);
      }
   }
   ...
   string ftxt = gUserPause?"PAUSED - press RESUME":
                 ...
                 ("QUANTUM INDEX ENGINE | "+gSym+" | THR "+DoubleToString(Signal_Threshold,0)+"% | M15:"+(Use_M15_Confirm?"ON":"OFF"));
