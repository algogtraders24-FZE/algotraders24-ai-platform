...
//|  OK: OIL-tuned defaults (ATR-based volatility handling)          |
...
#property description "NEXUSMINING EXPLORATION ALGO - Single-Asset Oil Trading System by ALGOTRADERS24 AI"
#property strict
...
#define EA_NAME    "NEXUSMINING EXPLORATION ALGO"
...
input group "=== SL-TP MODE (Oil-tuned) ==="
input bool   Use_Fixed_SLTP   = false;   // true=Fixed Points, false=ATR Dynamic
input int    Fixed_SL_Points  = 2000;    // Fixed SL points (OIL-tuned)
input int    Fixed_TP_Points  = 3500;    // Fixed TP points (OIL-tuned)
input double ATR_SL_Mult      = 2.2;     // OIL ATR x SL
input double ATR_TP_Mult      = 3.5;     // OIL ATR x TP
...
input int    Fixed_Trail_Pts  = 1500;    // Fixed trail points (OIL)
input bool   Use_BreakEven    = true;
input double MaxDrawdown_Pct  = 30.0;    // Max daily drawdown %
...
input int    Magic_Num   = 24010003;     // Unique per EA
input string EA_Tag      = "NEXUS_OIL";
...
void BuildDashboard()
{
   ...
   oText("tag",X+76,Y+48,"EXPLORATION ALGO . AI ENGINE",C'120,110,70',7);
   oText("core",X+W-92,Y+11,"CORE STATUS",C'90,138,132',7);
   oText("corev",X+W-92,Y+24,"* ONLINE",C'0,255,157',9,"Consolas",true);

   // Neural signal analysis
   int sy=Y+78;
   oText("siglbl",X+14,sy,"> NEURAL SIGNAL ANALYSIS",C'90,138,132',8,"Consolas",true);
...
void UpdateDashboard()
{
   ...
   if(!gReady){
      ...
      sT("sigpct","");
      sT("status","WARNING: "+EA_ASSET+" symbol not available - set Symbol_Override");
   ...
   if(gUserPause){ sBg("b_pause",C'80,0,0'); sT("status","PAUSED - press RESUME"); }
      ...
      sT("status","> AI ENGINE ACTIVE | "+gSym+" | "+(Use_Fixed_SLTP?"FIXED":"ATR")+" SL | THR "+DoubleToString(Signal_Threshold,0)+"% | M15:"+(Use_M15_Confirm?"ON":"OFF"));
   }
   ChartRedraw(0);
}
