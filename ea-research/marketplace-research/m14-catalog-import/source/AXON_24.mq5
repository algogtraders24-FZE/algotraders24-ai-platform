...
#property description "AXON 24 - Forex Major Pairs Multi-Pair System by ALGOTRADERS24 AI"
#property strict
...
input int    Max_Open_Trades  = 7;       // Max simultaneous open trades (7 pairs)
...
input int    Magic_Num   = 24020001;     // Unique per EA
input string EA_Tag      = "AXON24";
...
input bool   Show_Scanner   = true;     // Show major-pair signals on the dashboard
input bool   Multi_Trade    = true;     // Trade all detected major pairs (multi-pair mode)
input string Scan_Pair1     = "EURUSD"; // Major pair 1 (blank = skip)
input string Scan_Pair2     = "GBPUSD"; // Major pair 2 (blank = skip)
input string Scan_Pair3     = "USDJPY"; // Major pair 3 (blank = skip)
input string Scan_Pair4     = "AUDUSD"; // Major pair 4 (blank = skip)
input string Scan_Pair5     = "USDCAD"; // Major pair 5 (blank = skip)
input string Scan_Pair6     = "USDCHF"; // Major pair 6 (blank = skip)
input string Scan_Pair7     = "NZDUSD"; // Major pair 7 (blank = skip)

string  gScanName[7];     // resolved symbol names
double  gScanProb[7];     // last computed signal per asset (-1..1, or DATA_NR)
int     gScanCF[7];       // M15 confirm state per symbol (+1/-1/0)
string  gScanLabel[7]={"EURUSD","GBPUSD","USDJPY","AUDUSD","USDCAD","USDCHF","NZDUSD"};
datetime gScanLast=0;

// Canvas dashboard
CCanvas gCanvas;
bool    gCanvasReady=false;
double  gSpark[7][32];    // mini sparkline history per pair
int     gSparkCount[7]={0,0,0,0,0,0,0};
...
void DetectAndTune()
{
   // Axon 24 is a forex major-pair system: all pairs use the same input tuning.
   gAtrSL=ATR_SL_Mult; gAtrTP=ATR_TP_Mult; gTrailM=Trail_ATR_Mult; gDDLimit=MaxDrawdown_Pct;
   string s=gSym; StringToUpper(s);
   gAsset=s;   // show the pair name as the active profile
}
...
int OnInit()
{
   ...
   gInTester = inTester;
   gScanName[0]=Scan_Pair1;  gScanName[1]=Scan_Pair2;
   gScanName[2]=Scan_Pair3;  gScanName[3]=Scan_Pair4;
   gScanName[4]=Scan_Pair5;  gScanName[5]=Scan_Pair6;
   gScanName[6]=Scan_Pair7;
   for(int i=0;i<7;i++){
      gScanProb[i]=DATA_NR;
...
void UpdateScanner()
{
   ...
   gScanProb[4]=ScanSignal(gScanName[4]);
   gScanProb[5]=ScanSignal(gScanName[5]);
   gScanProb[6]=ScanSignal(gScanName[6]);
   for(int i=0;i<7;i++) gScanCF[i]=(gScanName[i]=="")?0:MConfirmSym(gScanName[i]);
   for(int i=0;i<7;i++) if(gScanProb[i]!=DATA_NR) SparkPush(i,gScanProb[i]);
}
...
// Build the list of pairs the user can manually trade (chart symbol + available scan pairs).
int BuildManualList(string &arr[])
{
   int c=0; ArrayResize(arr,8);
...
void UpdateDashboard()
{
   ...
   gCanvas.TextOut(14,12,"AXON 24",AX(CLR_ACCENT));
   gCanvas.FontSet("Arial Bold",10);
   gCanvas.TextOut(140,16,"FOREX MAJORS",AX((color)C'150,180,230'));
   ...
   // ---- right panel: 7-pair grid ----
   int rpx=162, rpy=56, rpw=W-rpx-6, rph=H-138;
   Panel(rpx,rpy,rpw,rph,AX((color)C'10,16,34',255),AX(CLR_ACCENT,160));
   gCanvas.FontSet("Arial Bold",10);
   gCanvas.TextOut(rpx+8,rpy+5,Multi_Trade?"PAIR GRID [LIVE TRADING]":"PAIR SCANNER",AX((color)C'190,210,240'));
   ...
   for(int i=0;i<7;i++){
      ...
      double p=gScanProb[i];
      Panel(rpx+6,ry,rpw-12,rowH-3,AX((color)C'14,22,40',255),AX((color)C'30,50,80',180));
      gCanvas.FontSet("Arial Bold",11);
      gCanvas.TextOut(rpx+12,ry+4,nm,AX((color)C'220,230,245'));
      if(gScanName[i]=="" || p==DATA_NR){
         gCanvas.FontSet("Arial",9);
         gCanvas.TextOut(rpx+90,ry+5,(gScanName[i]==""?"n/a":"wait"),AX((color)C'120,130,150'));
      } else {
         ...
         gCanvas.FontSet("Arial Bold",11);
         gCanvas.TextOut(rpx+90,ry+4,dir,dc);
         ...
         gCanvas.TextOut(rpx+140,ry+4,DoubleToString(relP,0)+"%"+m15,dc); }
         DrawSpark(rpx+rpw-86,ry+4,68,rowH-12,i,dc);
         // strength bar
         gCanvas.FillRectangle(rpx+12,ry+rowH-9,rpx+rpw-18,ry+rowH-6,AX((color)C'30,40,60'));
         int pw=(int)((rpw-30)*MathMin(1.0,MathAbs(p)));
         gCanvas.FillRectangle(rpx+12,ry+rowH-9,rpx+12+pw,ry+rowH-6,dc);
      }
   }
   ...
   string ftxt = gUserPause?"PAUSED - press RESUME":
                 ...
                 ("AXON 24 | "+gSym+" | THR "+DoubleToString(Signal_Threshold,0)+"% | M15:"+(Use_M15_Confirm?"ON":"OFF"));
