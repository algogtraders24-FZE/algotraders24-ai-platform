...
//|                  by  ALGOTRADERS24 AI                             |
//|    Multi-Stock Quantum Engine - Top 25 US Stocks       |
//|                                                                   |
//|  OK: Works on ALL brokers - auto prefix/suffix detection          |
//|  OK: Trades AAPL/MSFT/NVDA/TSLA/AMZN and 20 more       |
...
#property description "QUANTUM WALLSTREET PRO - Top 25 US Stocks Trading System by ALGOTRADERS24 AI"
#property strict
...
#define EA_NAME    "QUANTUM WALLSTREET PRO"
#define EA_COMPANY "ALGOTRADERS24 AI"
#define EA_ASSET   "STOCKS"
...
input group "=== ASSET ==="
input string Symbol_Override = "";       // Leave blank = trade the chart symbol. Or force one (e.g. AAPL, TSLA)

input group "=== QUANTUM PARAMETERS ==="
input double Signal_Threshold = 72;      // Min signal strength % to trade (0-100)
...
input bool   Trade_All_Pairs  = true;    // Trade every detected stock (multi-stock mode)

input group "=== SL-TP MODE ==="
input bool   Use_Fixed_SLTP   = false;   // false=ATR Dynamic (best for stocks), true=% of price
input double Stock_SL_Percent = 3.0;     // Fixed mode: SL = this % of stock price
input double Stock_TP_Percent = 5.0;     // Fixed mode: TP = this % of stock price
input int    Fixed_SL_Points  = 300;     // (legacy, unused for stocks)
input int    Fixed_TP_Points  = 500;     // (legacy, unused for stocks)
input double ATR_SL_Mult      = 3.0;     // ATR x SL multiplier (stocks)
input double ATR_TP_Mult      = 5.0;     // ATR x TP multiplier (stocks)
...
input int    Max_Open_Trades  = 10;      // Max simultaneous open trades (of 25 stocks)
input double MaxDrawdown_Pct  = 25.0;    // Max drawdown %
...
input int    Magic_Num   = 24060001;     // Unique per EA
input string EA_Tag      = "QWSPRO";
...
input group "=== MULTI-STOCK ENGINE ==="
...
input bool   Auto_Detect    = true;     // Auto-find the 25 US stocks at your broker
input string Custom_Pairs   = "";       // Optional: limit to some (e.g. AAPL,TSLA,NVDA). Blank=all 25.
...
string pfx = "QWS_";
color  CLR_ACCENT  = C'0,200,110';    // wall-street green
color  CLR_ACCENT2 = C'220,180,90';   // gold accent
color  CLR_BG      = C'8,14,11';        // deep market-black
color  CLR_PANEL   = C'12,20,16';       // panel

// Apply tuning to the chart symbol.
// Uses substring matching so it works with ANY broker prefix/suffix, e.g.
// Works with broker ticker naming: AAPL, AAPL.US, #AAPL, etc.
void DetectAndTune()
{
...
// Resolve an index by trying several broker aliases (NAS100/US100/USTEC/NDX100...).
// Returns the first tradable match. Skips dated futures (U6, Z6...) when a cash one exists.
bool ResolveIndexAliases(string &aliases[],string &out)
{
   // pass 1: exact alias match (prefer non-dated / cash symbols)
   for(int a=0;a<ArraySize(aliases);a++){
      if(SymbolSelect(aliases[a],true)){ out=aliases[a]; return true; }
   }
   // pass 2: scan all broker symbols for one CONTAINING an alias token
   int total=SymbolsTotal(false);
   for(int a=0;a<ArraySize(aliases);a++){
      string al=aliases[a]; StringToUpper(al);
      for(int i=0;i<total;i++){
         string sym=SymbolName(i,false);
         string u=sym; StringToUpper(u);
         if(StringFind(u,al)>=0){
...
// Each index: a clean label + the aliases brokers commonly use.
void IndexAliases(int which,string label,string &aliases[])
{
   // (label kept for clarity; aliases filled per index)
}

// Resolve a stock ticker by EXACT symbol match only (no substring scan).
// Prevents collisions like "BA" matching "BAC", or "MU" matching "MULN".
bool ResolveTickerExact(string &aliases[],string &out)
{
   for(int a=0;a<ArraySize(aliases);a++){
...
// Detect the 6 major indices at this broker (each tried via several aliases).
void DetectPairs()
{
   gPairCount=0;
   for(int i=0;i<30;i++){ gScanName[i]=""; gScanProb[i]=DATA_NR; gScanLabel[i]=""; gScanCF[i]=0; }
   if(gInTester) return;   // tester is single-symbol; skip multi-stock entirely

   // Top 25 liquid, volatile US stocks (tickers).
   string tickers[25] = {
      "AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA","AMD","NFLX","AVGO",
      "INTC","MU","QCOM","CRM","ORCL","PLTR","COIN","SOFI","JPM","BAC",
      "GS","DIS","BA","XOM","WMT"
   };

   // Optional custom list (comma-separated tickers)
   string only[]; int onlyN=0;
   if(Custom_Pairs!=""){
      string parts[]; int n=StringSplit(Custom_Pairs,',',parts);
      for(int i=0;i<n;i++){
         string t=parts[i]; StringTrimLeft(t); StringTrimRight(t); StringToUpper(t);
         if(t!=""){ ArrayResize(only,onlyN+1); only[onlyN]=t; onlyN++; }
      }
   }

   for(int k=0;k<25 && gPairCount<25;k++){
      string tk=tickers[k];
      // user limited the set? skip tickers not requested
      if(onlyN>0){
         bool req=false;
         for(int j=0;j<onlyN;j++) if(only[j]==tk) req=true;
         if(!req) continue;
      }
      // try ticker variants brokers use: AAPL, AAPL.US, #AAPL, AAPL.NAS, AAPL.NYSE, AAPLm
      string al[7];
      al[0]=tk; al[1]=tk+".US"; al[2]="#"+tk; al[3]=tk+".NAS";
      al[4]=tk+".NYSE"; al[5]=tk+"m"; al[6]=tk+".us";
      string resolved;
      if(ResolveTickerExact(al,resolved)){
         bool dup=false; for(int d=0;d<gPairCount;d++) if(gScanName[d]==resolved) dup=true;
         if(!dup){
            gScanName[gPairCount]=resolved;
            gScanLabel[gPairCount]=tk;     // show clean ticker
            gScanProb[gPairCount]=DATA_NR;
            gPairCount++;
         }
      }
   }
   Print("QUANTUM WALLSTREET PRO: detected ",gPairCount," tradable stocks at this broker");
}
...
void OnTick()
{
   ...
   if(cb!=0 && gLastBar!=0 && cb!=gLastBar){
      ...
      // Multi-stock auto-trading runs ONCE per new bar (not every tick),
      // so a fresh signal opens at most one new position per stock per bar.
...
void UpdateScanner()
{
   ...
   if(TimeCurrent()-gScanLast<12) return;   // refresh window (multi-index scan)
...
// Per-index ATR multipliers + a sensible fixed SL/TP (in index points) by label.
void ProfileFor(int idx,double &asl,double &atp,double &dd)
{
   // ATR-based tuning adapts to each index's own volatility, so multipliers are shared.
   asl=ATR_SL_Mult; atp=ATR_TP_Mult; dd=MaxDrawdown_Pct;
}

// Fixed SL/TP in POINTS, tuned per index (very different price scales).
// For stocks, a fixed "points" value makes no sense (prices range $20-$900).
// If fixed mode is on, derive SL/TP from a % of the stock's own price.
void IndexFixedPts(string label,double &slPts,double &tpPts)
{
   // not used for stocks; kept for compatibility (ATR mode is default)
   slPts=Fixed_SL_Points; tpPts=Fixed_TP_Points;
}

// SL/TP distance (in price) for a stock: % of current price when in fixed mode.
double StockFixedDist(string sym,bool isSL)
{
   double price=SymbolInfoDouble(sym,SYMBOL_BID);
   double pct=isSL?Stock_SL_Percent:Stock_TP_Percent;
...
void MultiExec(int idx,bool isBuy)
{
   ...
   double sd,td;
   if(Use_Fixed_SLTP){
      sd=StockFixedDist(sym,true); td=StockFixedDist(sym,false);
   } else {
...
// Run multi-index auto-trading across all detected indices.
void MultiTradeAll()
{
...
// Pre-trade check: returns true only if the symbol is inside its trading
// session (with a small edge margin). Skipped in the tester so validation
// runs cleanly. Prevents failed "market closed" orders in live trading.
bool CanTradeNow(string sym,ENUM_ORDER_TYPE otype,double lot,double price)
{
   // Skip the check in Strategy Tester / validation: the tester drives its own
   // market state, and extra trade-server calls here cause "not synchronized"
   // notices. Live trading still gets the full session guard below.
   if((bool)MQLInfoInteger(MQL_TESTER)) return true;

   // Market must be in a tradable state right now.
   long tm=SymbolInfoInteger(sym,SYMBOL_TRADE_MODE);
   if(tm==SYMBOL_TRADE_MODE_DISABLED || tm==SYMBOL_TRADE_MODE_CLOSEONLY)
      return false;

   // Confirm the current time is inside this symbol's trading session, with a
   // 60s margin from each edge (avoids the daily-break "market closed" instant).
...
bool MarketOpen(string sym)
{
   ...
   for(int s=0;s<8;s++){
      ...
      // (e.g. XAUUSD D1 bar close at 23:00) where a "market closed" order fails.
...
{
   if(!MarketOpen(gSym)) return;   // skip when market is closed (avoids failed orders)
   SetFilling();
   double price=isBuy?SymbolInfoDouble(gSym,SYMBOL_ASK):SymbolInfoDouble(gSym,SYMBOL_BID);
...
void ManualTrade(bool isBuy)
{
   ...
   if(!SymbolSelect(sym,true)) return;
   if(!MarketOpen(sym)){ Print("Manual trade skipped - market closed for ",sym); return; }
   SetFilling();
   double price=isBuy?SymbolInfoDouble(sym,SYMBOL_ASK):SymbolInfoDouble(sym,SYMBOL_BID);
   // SL/TP for the SELECTED symbol (% of price or ATR)
   double pt=SymbolInfoDouble(sym,SYMBOL_POINT);
   double sd,td;
   if(Use_Fixed_SLTP){ sd=StockFixedDist(sym,true); td=StockFixedDist(sym,false); }
   else {
...
void UpdateDashboard()
{
   ...
   // ---- header / status bar ----
   Panel(6,6,W-12,46,AX((color)C'26,20,12',255),AX(CLR_ACCENT,200));
   gCanvas.FontSet("Arial Bold",14);
   gCanvas.TextOut(14,11,"QUANTUM WALLSTREET PRO",AX(CLR_ACCENT));
   gCanvas.FontSet("Arial",9);
   gCanvas.TextOut(16,33,"TOP 25 US STOCKS . NEURAL ENGINE",AX((color)C'180,160,120'));
   ...
   gCanvas.TextOut(W-130,12,"STATUS",AX((color)C'180,160,120'));
   ...
   // ---- right panel: 25-stock grid (2 columns) ----
   int rpx=162, rpy=56, rpw=W-rpx-6, rph=H-138;
   Panel(rpx,rpy,rpw,rph,AX((color)C'10,16,34',255),AX(CLR_ACCENT,160));
   gCanvas.FontSet("Arial Bold",10);
   gCanvas.TextOut(rpx+8,rpy+4,(Multi_Trade?"STOCK GRID [LIVE]  ":"STOCK SCANNER  ")+IntegerToString(gPairCount)+" STOCKS",AX((color)C'190,210,240'));

   double thr=Signal_Threshold/100.0;
   int gridTop=rpy+22;
   int cols=2;
   int colW=(rpw-12)/cols;
   int perCol=13;                          // up to 13 rows x 2 cols = 26 (we show 25)
   int rowH=(rph-26)/perCol;
   int shown=(gPairCount>25)?25:gPairCount;
   for(int i=0;i<shown;i++){
      ...
      gCanvas.TextOut(cx+4,ry+1,nm,AX((color)C'210,222,238'));
      if(gScanName[i]=="" || p==DATA_NR){
         gCanvas.FontSet("Arial",8);
         gCanvas.TextOut(cx+colW-46,ry+1,"...",AX((color)C'110,125,150'));
      } else {
         ...
         gCanvas.FillRectangle(cx+56,ry+3,cx+colW-46,ry+9,AX((color)C'28,38,58'));
         int barW=(int)((colW-104)*MathMin(1.0,MathAbs(p)));
         gCanvas.FillRectangle(cx+56,ry+3,cx+56+barW,ry+9,dc);
         gCanvas.FontSet("Arial Bold",9);
         gCanvas.TextOut(cx+colW-42,ry+1,dir,dc);
      }
   }
   ...
   string ftxt = gUserPause?"PAUSED - press RESUME":
                 ...
                 ("QUANTUM WALLSTREET PRO | "+gSym+" | THR "+DoubleToString(Signal_Threshold,0)+"% | M15:"+(Use_M15_Confirm?"ON":"OFF"));
