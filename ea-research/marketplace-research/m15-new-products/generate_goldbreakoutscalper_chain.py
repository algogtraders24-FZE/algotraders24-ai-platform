"""
GoldBreakoutScalper (seller-provided EA) - real M3/M4/M5/M7 chain from
the real .xlsx Strategy Tester report, using M2-M7's own existing, real,
unmodified functions (exact same pattern as m12-gold-product-01/
generate_pdhpdl_evidence_chain.py). No new computation, no fabricated
data.

Run: python generate_goldbreakoutscalper_chain.py
"""
import glob
import json
import sys
...
def main() -> None:
    ...
    if not matches:
        raise FileNotFoundError(f"No real GoldBreakoutScalper Evidence package found in {REAL_EVIDENCE_DIR}")
    ...
    snapshot = {
        "_provenance": {
            "generatedBy": "generate_goldbreakoutscalper_chain.py (M15 seller-provided EA)",
