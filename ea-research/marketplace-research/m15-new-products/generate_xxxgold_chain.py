"""
XXX Gold (seller-provided EA) - real M3/M4/M5/M7 chain from the real
.xlsx Strategy Tester report, using M2-M7's own existing, real,
unmodified functions (exact same pattern as
m15-new-products/generate_btcethscalper_chain.py). No new computation,
no fabricated data.

Run: python generate_xxxgold_chain.py
"""
import glob
import json
import sys
...
def main() -> None:
    ...
    if not matches:
        raise FileNotFoundError(f"No real XXX Gold Evidence package found in {REAL_EVIDENCE_DIR}")
    ...
    snapshot = {
        "_provenance": {
            "generatedBy": "generate_xxxgold_chain.py (M15 seller-provided EA)",
