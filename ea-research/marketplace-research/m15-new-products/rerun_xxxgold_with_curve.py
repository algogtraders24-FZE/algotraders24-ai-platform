"""
Re-runs the real M3/M4/M5/M7 chain for XXX Gold using the REAL,
bar-level-curve-enriched evidence package (real M15 XAUUSD_EXNESS
candles filling the intrabar path between each of the 34,814 real
trades' own already-known entry/exit, via reconstruct_equity_curve.py
-- no invented numbers, every trade's floating P&L path is calibrated
to land exactly on that trade's own real grossProfit at exit; the
script already sums floating P&L across every simultaneously-open
real trade at each real M15 timestamp, so it handles this EA's 5
concurrent modules correctly). This directly addresses the
drawdown/recovery LIMITED gap honestly, with real data, rather than
relaxing the threshold further.

Run: python rerun_xxxgold_with_curve.py
"""
import json
import sys
...
EVIDENCE_PATH = HERE.parent / "m2-evidence-engine" / "real_evidence_output" / "evidence_XXXGOLD-v3.02-2026-BASELINE_bf8bfc841b143934.json"
SOURCE_ARTIFACT = Path(r"C:\Users\om\OneDrive\Desktop\xxx eas\ReportTester-5setfile.xlsx")
REGISTRY = HERE.parent / "m3-evidence-verification" / "version_registry.json"
EXPECTED_VERSION_ID = "XXXGOLD-v3.02-2026-BASELINE"
TRADING_SYSTEM_ID = "XXXGOLD"
OUTPUT_PATH = HERE / "xxxgold_chain_result.json"
...
def main() -> None:
    ...
    snapshot = {
        "_provenance": {
            "generatedBy": "rerun_xxxgold_with_curve.py (M15.1 real bar-level curve enrichment)",
