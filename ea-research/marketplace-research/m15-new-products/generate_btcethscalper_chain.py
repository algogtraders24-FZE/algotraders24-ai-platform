"""
BTC/ETH Crypto Scalper (seller-provided EA) - real M3/M4/M5/M7 chain from
the real .xlsx Strategy Tester report, using M2-M7's own existing, real,
unmodified functions (exact same pattern as
m15-new-products/generate_goldsilverscalper_chain.py). No new
computation, no fabricated data.

Run: python generate_btcethscalper_chain.py
"""
import glob
import json
import sys
...
SOURCE_ARTIFACT = Path(r"C:\Users\om\OneDrive\Desktop\staergy report\ReportTester-btceth.xlsx")
REGISTRY = HERE.parent / "m3-evidence-verification" / "version_registry.json"
EXPECTED_VERSION_ID = "BTCETHSCALPER-v1.00-2026-BASELINE"
TRADING_SYSTEM_ID = "BTCETHSCALPER"
OUTPUT_PATH = HERE / "btcethscalper_chain_result.json"


def main() -> None:
    matches = sorted(glob.glob(str(REAL_EVIDENCE_DIR / f"evidence_{EXPECTED_VERSION_ID}_*.json")))
    if not matches:
        raise FileNotFoundError(f"No real BTC/ETH Scalper Evidence package found in {REAL_EVIDENCE_DIR}")
    ...
    snapshot = {
        "_provenance": {
            "generatedBy": "generate_btcethscalper_chain.py (M15 seller-provided EA)",
