//+------------------------------------------------------------------+
//| Golden EMA Crossover — AUTO-GENERATED from spec        |
...
int OnInit()
  {
   h_emaFast = iMA(_Symbol, PERIOD_CURRENT, 20, 0, MODE_EMA, PRICE_CLOSE);
   h_emaSlow = iMA(_Symbol, PERIOD_CURRENT, 50, 0, MODE_EMA, PRICE_CLOSE);
...
void OpenTrade(int direction, double slDist, double tpDist)
  {
   ...
   req.deviation=20; req.magic=InpMagic; req.comment="GoldenEMACrossover"; req.type_filling=ORDER_FILLING_IOC;
   OrderSend(req, res);
  }
...
void OnTick()
  {
   ...
   double emaFast; if(!GetBuf(h_emaFast,1,emaFast)) return;
   double emaSlow; if(!GetBuf(h_emaSlow,1,emaSlow)) return;
   double atr14; if(!GetBuf(h_atr14,1,atr14)) return;
   double emaFast_prev; if(!GetBuf(h_emaFast,2,emaFast_prev)) return;
   double emaSlow_prev; if(!GetBuf(h_emaSlow,2,emaSlow_prev)) return;
