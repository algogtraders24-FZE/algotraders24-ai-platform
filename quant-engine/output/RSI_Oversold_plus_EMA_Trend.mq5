//+------------------------------------------------------------------+
//| RSI Oversold + EMA Trend — AUTO-GENERATED from spec        |
...
void OpenTrade(int direction, double slDist, double tpDist)
  {
   ...
   req.deviation=20; req.magic=InpMagic; req.comment="RSIOversold+EMATrend"; req.type_filling=ORDER_FILLING_IOC;
   OrderSend(req, res);
  }
...
void OnTick()
  {
   ...
   double ema50; if(!GetBuf(h_ema50,1,ema50)) return;
   double atr14; if(!GetBuf(h_atr14,1,atr14)) return;
   double rsi14_prev; if(!GetBuf(h_rsi14,2,rsi14_prev)) return;
   double ema50_prev; if(!GetBuf(h_ema50,2,ema50_prev)) return;
