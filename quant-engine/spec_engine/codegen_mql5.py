...
# Q1.4 Part 18 - the strategy `name` is free display text (unlike
# indicator ids, which schema.py now rejects outright if unsafe - see
# schema.py's ID_PATTERN) and is embedded into both a source comment and
# a quoted MQL string literal (req.comment="..."). A name containing `"`
# was confirmed to break out of that string literal and inject arbitrary
# compilable MQL (Q1.4_SECURITY_VALIDATION.md) - stripped here rather
# than rejected, since a display name has no reason to need those
# characters and rejecting it would be a worse user experience than
# quietly dropping unsafe punctuation from a label.
...
def generate_mql5(spec: dict) -> str:
    ...
    return f"""//+------------------------------------------------------------------+
//| {display_name} — AUTO-GENERATED from spec        |
