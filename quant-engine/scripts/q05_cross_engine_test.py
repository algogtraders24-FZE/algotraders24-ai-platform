"""
Q0.5 audit helper - temporary, isolated. Not production architecture; does
not modify any engine's semantics. Runs the same MACD Crossover / Bollinger
Mean Reversion specs, on the same underlying data, through all three
backtest engines (runner.py, execution_mtf.py, execution_tick.py), each
using quant_lite_risk_config() (BE/trailing/partial all OFF, per Q0.4),
each run twice for determinism, saving full trade ledgers for
reconciliation.

Data-fairness correction versus Q0.3/Q0.4: those scripts used the OLD,
non-Exness "XAUUSD" symbol (data_raw source, Aug 2020-Aug 2025) for signal
generation, paired with real Exness ticks for execution - two different
broker feeds sharing only a symbol name and an overlapping date range.
Q0.5 uses "XAUUSD_EXNESS" (the same real-Exness-tick-derived series) for
every engine's signal generation, so all three engines see genuinely the
same underlying market data, not just the same calendar dates.
"""
import json
import os
import sys
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "output", "q05")
