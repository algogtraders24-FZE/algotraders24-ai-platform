// components/agents/AgentTaskQueue.tsx
// Task queue list for an agent.

import type { AgentTask } from "@/types/agent";

const STATUS_COLOR: Record<AgentTask["status"], string> = {
  queued: "text-text-2",
  running: "text-gold",
  done: "text-success",
  failed: "text-danger",
};

export default function AgentTaskQueue({ tasks }: { tasks: AgentTask[] }) {
  return (
    <div className="rounded-xl border border-border bg-ink-2 p-4">
      <p className="mb-3 text-sm font-semibold text-text-2">Today's Tasks</p>
      <div className="space-y-2">
        {tasks.map((t) => (
          <div key={t.id} className="flex items-center justify-between text-xs">
            <span className="text-text-2">{t.title}</span>
            <span className={`font-semibold capitalize ${STATUS_COLOR[t.status]}`}>{t.status}</span>
          </div>
        ))}
        {tasks.length === 0 && <p className="text-xs text-text-3">No tasks.</p>}
      </div>
    </div>
  );
}