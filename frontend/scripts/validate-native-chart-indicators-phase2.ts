...
// MT5 feature-parity Phase 2 - expanded indicator library, first two
// entries: ATR and Stochastic Oscillator (MT5's real default 5/3/3
// "Slow Stochastic" - verified against mql5.com/metatrader5.com this
// session, not the textbook fast-stochastic every other platform
// defaults to). Standalone, assert-based verification, matching every
...
import { drawAtrPanel, drawStochasticPanel } from "../lib/chart-engine/sub-panel-renderer";
...
async function main(): Promise<void> {
  ...
  test("ATR and Stochastic are registered in PANEL_REGISTRY with their own real sub-panel row, never overlaid on price", () => {
    assert.ok(PANEL_REGISTRY.atr);
    assert.ok(PANEL_REGISTRY.stochastic);
    assert.equal(INDICATOR_PANEL_ID.atr, "atr");
    assert.equal(INDICATOR_PANEL_ID.stochastic, "stochastic");
  });

  test("DEFAULT_INDICATOR_CONFIGS' atr-14/stochastic-5-3-3 entries use MT5's real verified periods, and every entry's computed .panel matches INDICATOR_PANEL_ID (D2.7.5's own anti-drift guarantee, extended to Phase 2)", () => {
  ...
  test("renderChart end-to-end with ATR and Stochastic both active runs without throwing, using only canvas methods already relied on elsewhere", () => {
    const candles = makeCandles(60);
    const vp = fitToData(candles);
    const atrCfg = DEFAULT_INDICATOR_CONFIGS.find((c) => c.id === "atr")!;
    const stochCfg = DEFAULT_INDICATOR_CONFIGS.find((c) => c.id === "stochastic")!;
    const indicatorSeries = [computeIndicatorSeries(candles, atrCfg), computeIndicatorSeries(candles, stochCfg)];
    const ctx = fakeCtx();
    assert.doesNotThrow(() => {
      renderChart({
        ctx,
        dims: { width: 900, height: 700, priceAxisWidth: 64, timeAxisHeight: 22 },
        candles,
        viewport: vp,
        timeframe: "1h",
        crosshair: null,
        colors: resolveChartColors(),
        activePanels: ["atr", "stochastic"],
        indicatorSeries,
      });
    });
    assert.ok(!ctx.calls.includes("strokeRect"));
