function applySpreadAndSlippage(side, basePrice, spreadModel, slippageModel, instrumentSymbol, asOf, quantity) {
 ...
 */
export function resolveMarketFill(order, fillBar, spreadModel, slippageModel) {
 ...
 */
export function resolveLimitFill(order, fillBar) {
    const limit = order.limitPrice;
    if (order.side === "BUY") {
        if (fillBar.open <= limit)
            return { filled: true, fillPrice: fillBar.open, reason: "favorable gap: bar opened at or below the limit" };
        if (fillBar.low < limit)
            return { filled: true, fillPrice: limit, reason: "price traded through the limit (low < limit)" };
        return { filled: false, reason: "bar low did not trade strictly through the limit" };
    }
    if (fillBar.open >= limit)
        return { filled: true, fillPrice: fillBar.open, reason: "favorable gap: bar opened at or above the limit" };
    if (fillBar.high > limit)
        return { filled: true, fillPrice: limit, reason: "price traded through the limit (high > limit)" };
 ...
 */
export function resolveStopFill(order, fillBar) {
    const stop = order.stopPrice;
    if (order.side === "BUY") {
        if (fillBar.open >= stop)
            return { filled: true, fillPrice: fillBar.open, reason: "unfavorable gap: bar opened at or above the stop (gapped through)" };
        if (fillBar.high >= stop)
            return { filled: true, fillPrice: stop, reason: "price triggered the stop (high >= stop)" };
        return { filled: false, reason: "bar high did not reach the stop" };
    }
    if (fillBar.open <= stop)
        return { filled: true, fillPrice: fillBar.open, reason: "unfavorable gap: bar opened at or below the stop (gapped through)" };
    if (fillBar.low <= stop)
        return { filled: true, fillPrice: stop, reason: "price triggered the stop (low <= stop)" };
 ...
 */
export function resolveStopLimitFill(order, fillBar) {
    const stop = order.stopPrice;
    const limit = order.limitPrice;
    if (order.side === "BUY") {
...
function stopExitPrice(side, stop, bar) {
    if (side === "BUY")
        return bar.open <= stop ? bar.open : stop;
    return bar.open >= stop ? bar.open : stop;
}
function limitExitPrice(side, limit, bar) {
    if (side === "BUY")
 ...
 * this case.
 */
export function resolveProtectiveExit(side, stopLoss, takeProfit, bar) {
    if (stopLoss === undefined && takeProfit === undefined)
        return { exited: false, reason: "no protective levels set" };
