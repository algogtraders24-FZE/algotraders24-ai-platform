 ...
 * meaning.
 */
export function canonicalizeExpression(expr) {
    switch (expr.type) {
        case "comparison":
        ...
        case "logical": {
            ...
            if (expr.operator === "NOT")
                return { ...expr, operands: canonicalChildren };
...
function compareByCanonicalKey(a, b) {
    const ka = canonicalStringify(a);
    const kb = canonicalStringify(b);
    return ka < kb ? -1 : ka > kb ? 1 : 0;
}
function canonicalizeNamedCondition(c) {
...
function canonicalizeExit(exit) {
 ...
 */
export function canonicalizeStrategyIR(ir) {
    return {
        ...ir,
