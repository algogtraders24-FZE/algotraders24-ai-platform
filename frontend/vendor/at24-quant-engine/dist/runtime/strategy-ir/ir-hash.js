 ...
 * (Q0.7.36).
 */
export function computeCanonicalIRHash(ir) {
 ...
 */
export function computeCrossPlatformSemanticHash(ir) {
    const canonical = canonicalizeStrategyIR(ir);
    const { strategyId: _strategyId, strategyVersion: _strategyVersion, sourcePlatform: _sourcePlatform, sourceLanguage: _sourceLanguage, sourceVersion: _sourceVersion, sourceHash: _sourceHash, irVersion: _irVersion, metadata: _metadata, provenance: _provenance, ...semantic } = canonical;
    return computeCanonicalHash(semantic);
}
