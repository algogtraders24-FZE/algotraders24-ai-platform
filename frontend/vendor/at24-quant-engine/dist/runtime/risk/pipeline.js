...
function evaluateEntry(input, entry) {
    ...
    if (entry.stopLoss !== undefined && geometryViolations.length === 0) {
        ...
        if (!distanceResult.passed)
            violations.push(distanceResult.violation);
    }
    const maxPosResult = validateMaxPositionSize(input.riskSpecification, entry.quantity);
    if (!maxPosResult.passed)
        violations.push(maxPosResult.violation);
...
function evaluateManagement(input, position) {
    ...
    if (spec.partialClose) {
        const pc = evaluatePartialClose(spec.partialClose, input.direction, position.entryPrice, position.currentPrice, position.currentAtr, position.partialCloseAlreadyTriggered ?? false);
        if (pc.violation) {
