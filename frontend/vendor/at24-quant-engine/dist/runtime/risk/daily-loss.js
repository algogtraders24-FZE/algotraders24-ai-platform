 ...
 */
export function evaluateDailyLossLimit(spec, dailyLoss) {
    ...
    if (lossSoFar >= limitAmount) {
        return {
            passed: false,
            violation: makeViolation("DAILY_LOSS_LIMIT", "BLOCKING", `realized loss today (${lossSoFar}) >= configured limit (${limitAmount})`, lossSoFar, limitAmount, "AT_OR_BEYOND_LIMIT"),
        };
    }
    return { passed: true };
}
 ...
 */
export function computeTradingDayKey(timestampMs, dayBoundaryOffsetMinutes) {
    const shifted = timestampMs + dayBoundaryOffsetMinutes * 60_000;
    return new Date(shifted).toISOString().slice(0, 10);
}
export function isSameTradingDay(t1, t2, dayBoundaryOffsetMinutes) {
