 ...
 */
export function validateEntryGeometry(direction, entryPrice, stopLoss, takeProfit) {
    const violations = [];
    if (!Number.isFinite(entryPrice) || entryPrice <= 0) {
        violations.push(makeViolation("INVALID_RISK_DISTANCE", "BLOCKING", `entryPrice ${entryPrice} must be a finite number > 0`, entryPrice, 0, "INVALID_NUMERIC_VALUE"));
    ...
    if (stopLoss !== undefined) {
        if (!Number.isFinite(stopLoss) || stopLoss <= 0) {
            ...
            violations.push(makeViolation("INVALID_STOP", "BLOCKING", `BUY stopLoss (${stopLoss}) must be below entry (${entryPrice})`, stopLoss, entryPrice, "INVALID_DIRECTION_RELATIVE_TO_ENTRY"));
        }
        else if (direction === "SELL" && stopLoss <= entryPrice) {
            violations.push(makeViolation("INVALID_STOP", "BLOCKING", `SELL stopLoss (${stopLoss}) must be above entry (${entryPrice})`, stopLoss, entryPrice, "INVALID_DIRECTION_RELATIVE_TO_ENTRY"));
        }
    }
    if (takeProfit !== undefined) {
        if (!Number.isFinite(takeProfit) || takeProfit <= 0) {
            violations.push(makeViolation("INVALID_TARGET", "BLOCKING", `takeProfit ${takeProfit} must be a finite number > 0`, takeProfit, 0, "INVALID_NUMERIC_VALUE"));
        }
        else if (direction === "BUY" && takeProfit <= entryPrice) {
            violations.push(makeViolation("INVALID_TARGET", "BLOCKING", `BUY takeProfit (${takeProfit}) must be above entry (${entryPrice})`, takeProfit, entryPrice, "INVALID_DIRECTION_RELATIVE_TO_ENTRY"));
        }
        else if (direction === "SELL" && takeProfit >= entryPrice) {
            violations.push(makeViolation("INVALID_TARGET", "BLOCKING", `SELL takeProfit (${takeProfit}) must be below entry (${entryPrice})`, takeProfit, entryPrice, "INVALID_DIRECTION_RELATIVE_TO_ENTRY"));
        }
    }
    return violations;
}
/** BUY: entry - stop. SELL: stop - entry. Positive by construction for a valid risk. */
export function computeRiskDistance(direction, entryPrice, stopLoss) {
    return direction === "BUY" ? entryPrice - stopLoss : stopLoss - entryPrice;
}
export function validateRiskDistance(direction, entryPrice, stopLoss) {
