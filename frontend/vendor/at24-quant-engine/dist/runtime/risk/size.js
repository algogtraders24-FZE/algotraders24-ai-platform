...
export function validateProposedSize(quantity) {
...
export function validateMaxPositionSize(spec, quantity) {
    if (spec.maxPositionSize === undefined)
        return { passed: true };
    if (quantity > spec.maxPositionSize) {
        return {
            passed: false,
            violation: makeViolation("MAX_POSITION", "BLOCKING", `proposed size (${quantity}) exceeds maxPositionSize (${spec.maxPositionSize})`, quantity, spec.maxPositionSize, "EXCEEDS_MAXIMUM"),
        };
    }
    return { passed: true };
}
