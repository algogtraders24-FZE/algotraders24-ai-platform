 ...
 *   refuses to pass.
 */
export function mqlImportLifecycleStages(output) {
    const blocking = blockingDiagnostics(output.report);
    const IMPORTED = { stage: "IMPORTED", outcome: "PASSED", detail: `source accepted, sourceHash ${output.document.sourceHash.slice(0, 16)}...` };
    if (blocking.length > 0) {
        const detail = blocking.map((d) => `${d.code}: ${d.message}`).join("; ");
        const failed = { stage: "PARSED", outcome: "FAILED", detail };
        return {
            IMPORTED,
            PARSED: failed,
    ...
    const PARSED = { stage: "PARSED", outcome: "PASSED", detail: `${output.report.diagnostics.length} diagnostic(s), zero BLOCKING` };
    const structural = validateStrategyIRStructure(output.ir);
    if (!structural.valid) {
        const detail = structural.errors.join("; ");
        const failed = { stage: "IR_VALID", outcome: "FAILED", detail };
    ...
    const IR_VALID = { stage: "IR_VALID", outcome: "PASSED" };
