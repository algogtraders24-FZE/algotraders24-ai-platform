...
const MQL4_EVENT_MAP = { init: "MQL4_INIT", start: "MQL4_START", deinit: "MQL4_DEINIT" };
const SERIES_IDENTIFIERS = { Close: "CLOSE", Open: "OPEN", High: "HIGH", Low: "LOW", Volume: "VOLUME" };
...
const INDICATOR_FUNCTIONS = { iMACD: "MACD", iBands: "BOLLINGER_BANDS" };
...
function argText(arg) {
    ...
    if (arg.kind === "UnaryExpression")
        return `${arg.operator}${argText(arg.argument) ?? ""}`;
    return undefined;
}
function isSymbolOrTimeframeLiteralish(arg) {
    if (!arg)
        return false;
...
class SemanticAnalyzer {
    ...
    diagnostics = [];
    eventHandlers = [];
    barTickModesByFunction = new Map();
    newBarDetectionSites = [];
    seriesReferences = [];
    indicatorCalls = [];
    crossPatterns = [];
    orderCalls = [];
    ...
    partialCloseCalls = [];
    sessionTimeCalls = [];
    symbolTimeframeSites = [];
    unsupportedConstructs = [];
    simpleEntryConditions = [];
    managementPatterns = [];
    pendingOrderManagementCalls = [];
    globalReadPositions = new Map();
    globalWritePositions = new Map();
    ...
    recordedPendingManagementPositions = new Set();
    functionsReadingBidAsk = new Set();
    ...
    variableIndicatorBindings = new Map();
    ...
    variableRiskBindings = new Map();
    constructor(dialect, globals, functions, inputDefaults = new Map()) {
    ...
    pushUnsupported(category, functionName, position) {
        this.unsupportedConstructs.push({ category, functionName, position });
    }
    recordGlobalAccess(name, position, isWrite) {
        const map = isWrite ? this.globalWritePositions : this.globalReadPositions;
        map.get(name)?.push(position);
    }
    analyzeFunction(fn) {
        if (MQL5_EVENT_MAP[fn.name])
            this.eventHandlers.push({ kind: MQL5_EVENT_MAP[fn.name], functionName: fn.name, position: fn.position });
        if (MQL4_EVENT_MAP[fn.name])
            this.eventHandlers.push({ kind: MQL4_EVENT_MAP[fn.name], functionName: fn.name, position: fn.position });
    ...
    resolveIndicatorCallReference(expr) {
        const name = calleeName(expr.callee);
        if (!name)
            return undefined;
        if (name === "iMA") {
            const methodArg = argText(expr.args[4]);
            const family = methodArg === "MODE_EMA" ? "EMA" : methodArg === "MODE_SMA" ? "SMA" : undefined;
            if (!family)
                return undefined;
    ...
    resolveIndicatorOperand(expr) {
        const direct = this.indicatorCallValue(expr);
        if (direct)
            return direct;
        if (expr.kind === "Identifier")
            return this.variableIndicatorBindings.get(expr.name);
        return undefined;
    }
            ...
            if (a.kind === "Identifier" && b.kind === "Literal" && typeof b.value === "number") {
                ...
                if (binding?.family === "ATR") {
                    ...
                    if (Number.isFinite(period))
                        return { atrMultiple: b.value, atrPeriod: period };
                }
            }
        }
        return undefined;
    ...
    resolveRiskLegBinding(expr) {
        if (expr.kind !== "BinaryExpression" || (expr.operator !== "+" && expr.operator !== "-"))
            return undefined;
        for (const side of [expr.left, expr.right]) {
            const atrTerm = this.resolveAtrMultipleTerm(side);
            if (atrTerm)
                return { kind: "atr-multiple", ...atrTerm };
        }
        for (const side of [expr.left, expr.right]) {
            if (side.kind === "Literal" && typeof side.value === "number")
                return { kind: "fixed-distance", distance: Math.abs(side.value) };
        }
        return undefined;
    }
    ...
    resolveEntryConditionOperand(expr) {
        const indicatorRef = this.resolveIndicatorOperand(expr);
        if (indicatorRef)
            return { kind: "indicator", ref: indicatorRef };
    ...
    detectSimpleEntryCondition(test, consequent) {
        if (test.kind !== "BinaryExpression" || !this.comparisonOperators.has(test.operator))
            return;
    ...
    findOrderDirectionIn(stmt) {
        ...
        const visitExpr = (e) => {
            ...
            if (e.kind === "CallExpression") {
                ...
                if (name === "OrderSend") {
                    ...
                    if (cmd?.startsWith("OP_BUY"))
                        found = "BUY";
                    else if (cmd?.startsWith("OP_SELL"))
                        found = "SELL";
                    return;
                }
            ...
            else if (e.kind === "UnaryExpression")
        ...
        const visitStmt = (s) => {
            ...
            if (s.kind === "BlockStatement")
                ...
                if (s.alternate)
                    visitStmt(s.alternate);
            }
            else if (s.kind === "ExpressionStatement")
                visitExpr(s.expression);
            else if (s.kind === "VariableDeclarationStatement")
                s.declarators.forEach((d) => d.initializer && visitExpr(d.initializer));
        };
        visitStmt(stmt);
        return found;
    }
    ...
    isEntryPriceExpr(e) {
        if (e.kind !== "CallExpression")
            return false;
        const name = calleeName(e.callee);
        if (name === "OrderOpenPrice")
            return true;
        if (name === "PositionGetDouble" && e.args.some((a) => argText(a)?.includes("POSITION_PRICE_OPEN")))
            return true;
        return false;
    }
    /**
     ...
     */
    resolveManagementDistance(expr) {
        if (this.isEntryPriceExpr(expr))
            return { binding: { kind: "fixed-distance", distance: 0 }, baseIsEntryPrice: true };
        if (expr.kind !== "BinaryExpression" || (expr.operator !== "+" && expr.operator !== "-"))
            return undefined;
    ...
    resolveFavorableTriggerDistance(test) {
        if (test.kind !== "BinaryExpression" || (test.operator !== ">=" && test.operator !== ">"))
            return undefined;
        if (test.right.kind !== "Literal" || typeof test.right.value !== "number")
            return undefined;
        if (test.left.kind !== "BinaryExpression" || test.left.operator !== "-")
            return undefined;
        ...
        if (leftIsEntry === rightIsEntry)
            return undefined; // both or neither reference entry price -> not provable
    ...
    resolveHoldingDurationTrigger(test) {
        if (test.kind !== "BinaryExpression" || (test.operator !== ">=" && test.operator !== ">"))
            return undefined;
        if (test.right.kind !== "Literal" || typeof test.right.value !== "number")
            return undefined;
        ...
        if (leftName !== "TimeCurrent")
            return undefined;
        if (rightName !== "OrderOpenTime" && rightName !== "PositionGetInteger")
            return undefined;
    ...
    resolvePartialCloseFraction(volumeExpr) {
        if (volumeExpr.kind !== "BinaryExpression" || volumeExpr.operator !== "*")
            return undefined;
        const isLotsExpr = (e) => {
            if (e.kind !== "CallExpression")
                return false;
            ...
            if (name === "OrderLots")
                return true;
            if (name === "PositionGetDouble" && e.args.some((a) => argText(a)?.includes("POSITION_VOLUME")))
                return true;
            return false;
        };
        for (const [a, b] of [
    ...
    static MODIFY_FUNCTIONS = new Set(["OrderModify", "PositionModify"]);
    static CLOSE_FUNCTIONS = new Set(["OrderClose", "PositionClosePartial", "PositionClose"]);
    /**
     ...
     */
    detectManagementPattern(test, consequent) {
    ...
    // -------------------------------------------------------------------
    static PENDING_MGMT_CALL_NAMES = new Set(["OrderModify", "OrderDelete", "PositionModify", "PositionClose"]);
    ...
    resolveOrderTypeFilter(test) {
        if (test.kind !== "BinaryExpression" || test.operator !== "==")
            return undefined;
        const isTypeQuery = (e) => e.kind === "CallExpression" && (calleeName(e.callee) === "OrderType" || (calleeName(e.callee) === "PositionGetInteger" && e.args.some((a) => argText(a)?.includes("POSITION_TYPE"))));
        const constText = (e) => (e.kind === "Identifier" ? e.name : undefined);
        if (isTypeQuery(test.left))
            return constText(test.right);
        if (isTypeQuery(test.right))
            return constText(test.left);
        return undefined;
    }
    ...
    resolvePendingOrderTarget(argExpr) {
        if (!argExpr)
            return { kind: "UNKNOWN" };
        if (argExpr.kind === "Identifier" && argExpr.name === "_Symbol")
            return { kind: "SYMBOL", sourceExpr: argExpr.name };
        if (argExpr.kind === "CallExpression" && calleeName(argExpr.callee) === "Symbol")
            return { kind: "SYMBOL", sourceExpr: "Symbol()" };
        if (argExpr.kind === "Literal" && typeof argExpr.value === "string")
            return { kind: "SYMBOL", sourceExpr: String(argExpr.value) };
        if (argExpr.kind === "Identifier")
            return { kind: "TICKET", sourceExpr: argExpr.name };
        const text = argText(argExpr);
        return { kind: "UNKNOWN", ...(text !== undefined ? { sourceExpr: text } : {}) };
    }
    resolvePendingOrderCondition(test) {
        const orderTypeConstant = this.resolveOrderTypeFilter(test);
        if (orderTypeConstant !== undefined)
            return { kind: "ORDER_TYPE_FILTER", orderTypeConstant, sourceExpr: this.describeExpr(test) };
        const triggerDistance = this.resolveFavorableTriggerDistance(test);
        if (triggerDistance !== undefined)
            return { kind: "FAVORABLE_DISTANCE", favorableTriggerDistance: { kind: "fixed-distance", distance: triggerDistance }, sourceExpr: this.describeExpr(test) };
     ...
     */
    classifyPendingManagementCall(callExpr) {
        const name = calleeName(callExpr.callee);
        if (!name)
            return undefined;
        const isMethodCall = callExpr.callee.kind === "MemberExpression";
        if (name === "OrderDelete")
            return { functionName: isMethodCall ? "CTrade.OrderDelete" : "OrderDelete", targetArgIndex: 0 };
        if (name === "PositionClose")
            return { functionName: isMethodCall ? "CTrade.PositionClose" : "PositionClose", targetArgIndex: 0 };
        if (name === "PositionModify")
            return { functionName: isMethodCall ? "CTrade.PositionModify" : "PositionModify", targetArgIndex: 0 };
        if (name === "OrderModify") {
            return isMethodCall
    ...
    buildPendingOrderManagementCallSite(callExpr, classified, condition) {
     ...
     */
    detectPendingOrderManagementConditional(test, consequent) {
    ...
    recordUnconditionalPendingManagementIfNeeded(expr, _pos) {
        const key = `${expr.position.line}:${expr.position.column}`;
        if (this.recordedPendingManagementPositions.has(key))
            return;
    ...
    classifyBarTickMode(fn) {
        if (fn.name === "OnTimer")
            return "TIMER";
        if (fn.name === "OnTrade" || fn.name === "OnTradeTransaction")
            return "TRADE_EVENT";
        if (fn.name === "OnTick" || fn.name === "start")
            return "TICK"; // never assumed to mean bar-close (Q0.8.9) — refined below once new-bar sites are known
        return "UNKNOWN";
    }
    /** Q0.8.36 — does this LOCALLY-DEFINED function's body read live bid/ask anywhere (transitively, one hop)? */
    functionReadsBidAsk(fnName) {
        if (this.functionsReadingBidAsk.has(fnName))
            return true;
        ...
        const visitExpr = (e) => {
            if (e.kind === "CallExpression") {
                const name = calleeName(e.callee);
                if (name === "SymbolInfoDouble" && e.args.some((a) => argText(a) === "SYMBOL_BID" || argText(a) === "SYMBOL_ASK"))
                    found = true;
                visitExpr(e.callee);
                e.args.forEach(visitExpr);
            }
        ...
        const visitStmt = (s) => {
            if (s.kind === "BlockStatement")
                ...
                visitExpr(s.expression);
            else if (s.kind === "ReturnStatement" && s.argument)
        ...
        visitStmt(fn.body);
        if (found)
            this.functionsReadingBidAsk.add(fnName);
        return found;
    }
    /** Q0.8.23's "top-level, unconditional call" detection for the realtime-dependency heuristic (Q0.8.35). */
    checkUnconditionalRealtimeCalls(fnName, body) {
        if (fnName !== "OnTick" && fnName !== "start")
            return;
    ...
    walkStatement(stmt, fnName, _guardedByNewBar) {
        switch (stmt.kind) {
            case "BlockStatement":
            ...
            case "IfStatement": {
                ...
                if (stmt.alternate)
                    this.walkStatement(stmt.alternate, fnName, _guardedByNewBar);
                return;
            }
            case "ForStatement":
                if (stmt.init)
                    this.walkStatement(stmt.init, fnName, _guardedByNewBar);
                if (stmt.test)
                    this.walkExpression(stmt.test, fnName);
                if (stmt.update)
                    this.walkExpression(stmt.update, fnName);
            ...
            case "ReturnStatement":
                if (stmt.argument)
                    this.walkExpression(stmt.argument, fnName);
                return;
    ...
    testLooksLikeNewBarGuard(test, fnName) {
        if (test.kind !== "CallExpression")
            return false;
        ...
        switch (expr.kind) {
            ...
            case "AssignmentExpression":
                ...
                if (expr.target.kind === "Identifier") {
                    ...
                    this.walkExpression(expr.target, fnName);
            ...
            case "CallExpression":
                ...
                return;
            case "Identifier":
                if (this.globalReadPositions.has(expr.name))
                    this.recordGlobalAccess(expr.name, expr.position, false);
                return;
            case "Literal":
                return;
        }
    ...
    detectTimeComparisonNewBar(expr, _fnName) {
        if (expr.operator !== "!=" && expr.operator !== "==")
            return;
        const isTimeIndex = (e) => e.kind === "IndexExpression" && e.object.kind === "Identifier" && e.object.name === "Time";
    ...
    detectCrossPattern(expr, _fnName) {
        if (expr.operator !== "&&")
            return;
        const l = expr.left;
        const r = expr.right;
        if (l.kind !== "BinaryExpression" || r.kind !== "BinaryExpression")
            return;
        const seriesKey = (e) => {
            if (e.kind === "IndexExpression" && e.object.kind === "Identifier")
                return e.object.name;
            if (e.kind === "CallExpression")
                return calleeName(e.callee);
            return undefined;
        };
        const offsetOf = (e) => (e.kind === "IndexExpression" && e.index.kind === "Literal" && typeof e.index.value === "number" ? e.index.value : undefined);
    ...
    detectSeriesIndex(expr, _fnName) {
    ...
    detectCall(expr, fnName) {
        ...
        if (name === "iCustom")
            return this.pushUnsupported("ICUSTOM", name, pos);
        if (name === "WebRequest")
            return this.pushUnsupported("WEBREQUEST", name, pos);
        if (name === "FileOpen" || name === "FileWrite" || name === "FileRead")
            return this.pushUnsupported("EXTERNAL_FILE", name, pos);
        ...
        if (name === "OrderSend") {
            ...
            const pendingOrderType = isLimit ? "LIMIT" : isStop ? "STOP" : side !== undefined ? "MARKET" : undefined;
        ...
        if ((name === "BuyLimit" || name === "SellLimit" || name === "BuyStop" || name === "SellStop") && expr.callee.kind === "MemberExpression") {
            const side = name.startsWith("Buy") ? "BUY" : "SELL";
            const pendingOrderType = name.endsWith("Limit") ? "LIMIT" : "STOP";
            const volumeExpr = argText(expr.args[0]);
        ...
        if (SESSION_TIME_FUNCTIONS.has(name)) {
            ...
            if (isLocalTime)
                this.diagnostics.push(diagnostic("LOCAL_TIME_USED", "TimeLocal() depends on the machine's local clock, not broker/server time or UTC — recorded as platform-dependent, never silently treated as UTC (Q0.8.30)", "WARNING", pos));
        ...
        if (this.functions.some((f) => f.name === name))
            return;
 ...
 */
export function analyzeMQLSemantics(program, dialect) {
    const globals = program.body.filter((n) => n.kind === "GlobalVariableDeclaration");
    const functions = program.body.filter((n) => n.kind === "FunctionDeclaration");
