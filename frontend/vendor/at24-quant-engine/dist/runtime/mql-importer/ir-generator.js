...
import { comparison, literal, indicatorOperand } from "../../domain/expression.js";
...
const MQL_TYPE_TO_PARAM_TYPE = {
    int: "number", long: "number", double: "number", float: "number", short: "number", uint: "number", ulong: "number", short2: "number",
    bool: "boolean",
    string: "string",
};
function periodConstToTimeframe(period) {
    const map = { PERIOD_M1: "M1", PERIOD_M5: "M5", PERIOD_M15: "M15", PERIOD_M30: "M30", PERIOD_H1: "H1", PERIOD_H4: "H4", PERIOD_D1: "D1", PERIOD_W1: "W1", PERIOD_MN1: "MN1" };
    return map[period];
}
function parameterFromInput(node) {
 ...
 */
export function generateStrategyIR(document, program, model, parseDiagnostics, semanticDiagnostics, options) {
    const unsupportedSemantics = [];
    const approximations = [];
    const sourceToIRMappings = [];
    function markUnsupported(feature, reason, severity, executionImpact, line) {
    ...
    function markApproximation(feature, original, replacement, difference, impact) {
        approximations.push({ feature, original, replacement, difference, impact });
    }
    function traceMapping(irFeature, sourceLine, sourceColumn) {
        sourceToIRMappings.push({ irFeature, sourceLine, sourceColumn, sourceHash: document.sourceHash });
    }
    const inputNodes = program.body.filter((n) => n.kind === "InputDeclaration");
    const parameters = [];
    ...
    if (otherTimeframes.length > 0 && !hasNewBarSignal) {
        ...
        markUnsupported("multi-timeframe HTF availability", "HTF usage appears to be gated by a plausibly-named (but cross-file, unverified) new-bar check", "WARNING", "availability is recorded as HTF_CLOSE_AVAILABLE on trust in the naming convention, not independently proven");
    }
    const timeframeSeries = [options.executionTimeframe, ...otherTimeframes].map((tf) => {
        const isExecution = tf === options.executionTimeframe;
        const role = isExecution ? "EXECUTION" : timeframeDurationMs(tf) > timeframeDurationMs(options.executionTimeframe) ? "HIGHER" : "LOWER";
        return { timeframe: tf, role, availabilityPolicy: "HTF_CLOSE_AVAILABLE", alignmentPolicy: "CLOSE_ALIGNED" };
    });
    ...
    const sizingModel = riskPercentInput
        ? ({ method: "percent-equity-risk", percent: riskPercentInput.defaultValue.kind === "Literal" ? Number(riskPercentInput.defaultValue.value) : 1 })
        : ({ method: "fixed-quantity", quantity: 1 });
    if (riskPercentInput) {
        markApproximation("position sizing method", `input "${riskPercentInput.name}" (percent-of-equity risk input) feeds an unresolved cross-file sizing function`, `PositionSizingMethod { method: "percent-equity-risk", percent: ${sizingModel.method === "percent-equity-risk" ? sizingModel.percent : 0} }`, "the actual lot-size FORMULA (including any broker volume-step rounding) lives in an unanalyzed #include and is not verified", "simulated position size may differ from the real EA's if the unresolved formula diverges from plain percent-equity-risk");
    }
    /**
     ...
     * unchanged from Q0.8.
     */
    function toConditionOperand(side) {
        if (side.kind === "literal")
            return literal(side.value);
     ...
     */
    function resolvePriceExprToReference(exprText) {
        if (exprText === undefined)
            return undefined;
        const trimmed = exprText.trim();
        if (trimmed === "Bid")
            return { kind: "UNSUPPORTED", reason: "BID" };
        if (trimmed === "Ask")
            return { kind: "UNSUPPORTED", reason: "ASK" };
    ...
    function buildOrderTypeFields(direction) {
        ...
        if (missingRequiredPrice || limitPrice?.kind === "UNSUPPORTED" || stopPrice?.kind === "UNSUPPORTED") {
            markUnsupported(`${pendingOrderType} order price`, missingRequiredPrice
                ...
                : `a ${pendingOrderType} order references a live Bid/Ask price, which is not deterministically computable without a bid/ask feed (Q0.11.3)`, "BLOCKING", "this entry's executionType/limitPrice/stopPrice are left as detected but unresolved rather than fabricated");
        }
        return { executionType: pendingOrderType, ...(limitPrice ? { limitPrice } : {}), ...(stopPrice ? { stopPrice } : {}) };
    }
    function buildRealConditionEntry(direction, id) {
        ...
        return {
            id,
            direction,
            condition: comparison(site.operator, toConditionOperand(site.left), toConditionOperand(site.right)),
            sizingModel,
            timing: "NEXT_BAR_OPEN",
            ...buildOrderTypeFields(direction),
        };
    }
    const buyEntry = hasBuy ? (buildRealConditionEntry("BUY", "entry-buy") ?? { id: "entry-buy", direction: "BUY", condition: UNREPRESENTABLE_CONDITION, sizingModel, timing: "INTRABAR", ...buildOrderTypeFields("BUY") }) : undefined;
    const sellEntry = hasSell ? (buildRealConditionEntry("SELL", "entry-sell") ?? { id: "entry-sell", direction: "SELL", condition: UNREPRESENTABLE_CONDITION, sizingModel, timing: "INTRABAR", ...buildOrderTypeFields("SELL") }) : undefined;
    if (buyEntry)
    ...
    if (anyPlaceholderEntry) {
        markUnsupported("entry/exit signal logic", "the real entry sequence could not be proven to match the one entry-condition shape this importer reconstructs (`if (indicatorA <op> indicatorB) { order }`) — it may be a multi-bar state machine spanning several #include modules, or a comparison this importer cannot resolve to indicator operands", "BLOCKING", "the affected entries[].condition is a placeholder that always evaluates false — this IR cannot be executed until a future sprint extends condition reconstruction or adds cross-bar state-machine representation");
    }
    ...
    const isMeaningfulPriceExpr = (expr) => expr !== undefined && expr !== "0" && expr !== "0.0";
    ...
    function bindingToRule(binding, kind) {
        if (!binding)
            return undefined;
        if (kind === "stopLoss") {
     ...
     */
    function registerAtrIndicatorIfNeeded(binding) {
        if (binding?.kind === "atr-multiple")
            resolvedIndicators.push({ kind: "named", family: "ATR", params: [binding.atrPeriod] });
    }
    ...
    if (hasSLTP && !sltpFullyResolved) {
        markUnsupported("stop-loss / take-profit values", "SL/TP are computed by unresolved cross-file functions; the resulting price cannot be expressed as a StopLossRule/TakeProfitRule without resolving that formula", "BLOCKING", "risk.stopLoss/risk.takeProfit are left unset rather than fabricated");
    }
    ...
    function bindingToDistanceSpec(binding) {
    ...
    if (partialClosePattern)
        registerAtrIndicatorIfNeeded(partialClosePattern.triggerDistance);
    if (breakevenRule)
        markUnsupported("breakeven rule", "reconstructed from a provable `if (<price> - OrderOpenPrice() >= trigger) OrderModify(..., OrderOpenPrice() [+/- offset], ...)` source pattern", "WARNING", "the reconstruction assumes this is the ONLY place the stop is modified for this purpose — a second, unrecognized modify site elsewhere in the source would not be reflected");
    if (trailingStopRule)
        markUnsupported("trailing-stop rule", "reconstructed from a provable `if (<price> - OrderOpenPrice() >= trigger) OrderModify(..., <currentPrice> +/- distance, ...)` source pattern", "WARNING", "the reconstruction assumes this is the ONLY place the stop is modified for this purpose");
    if (partialCloseRule)
        markUnsupported("partial-close rule", "reconstructed from a provable `if (<price> - OrderOpenPrice() >= trigger) OrderClose(ticket, lots*fraction, ...)` source pattern", "WARNING", "only a single fractional close site is represented; a strategy with multiple distinct partial-close thresholds would only reflect the first one found");
    if (maxHoldingRule)
        markUnsupported("max-holding-period rule", "reconstructed from a provable `if (TimeCurrent() - OrderOpenTime() >= seconds) OrderClose(...)` source pattern", "WARNING", "assumes this is the strategy's only time-based exit condition");
    ...
    function pendingMgmtBindingToDistanceSpec(binding) {
        return binding.kind === "atr-multiple" ? { mode: "atr-multiple", atrMultiple: binding.atrMultiple, atrPeriod: binding.atrPeriod } : { mode: "absolute", value: binding.distance };
    }
    function toTargetIR(site) {
    ...
    function toConditionIR(site) {
        const c = site.condition;
        if (c.kind === "UNCONDITIONAL")
            return { kind: "ALWAYS", provable: true, ...(c.sourceExpr !== undefined ? { sourceExpr: c.sourceExpr } : {}) };
        if (c.kind === "ORDER_TYPE_FILTER")
            return { kind: "ORDER_TYPE_FILTER", provable: true, ...(c.orderTypeConstant !== undefined ? { orderTypeConstant: c.orderTypeConstant } : {}), ...(c.sourceExpr !== undefined ? { sourceExpr: c.sourceExpr } : {}) };
        if (c.kind === "FAVORABLE_DISTANCE" && c.favorableTriggerDistance)
            return { kind: "FAVORABLE_DISTANCE", provable: true, distance: pendingMgmtBindingToDistanceSpec(c.favorableTriggerDistance), ...(c.sourceExpr !== undefined ? { sourceExpr: c.sourceExpr } : {}) };
    ...
    const pendingOrderManagementRules = model.pendingOrderManagementCalls.map((site, index) => {
        ...
        const semanticFidelity = isPositionLevelCall ? "UNKNOWN" : target.provable && condition.provable && operation.kind !== "UNKNOWN" ? "EXACT" : "UNKNOWN";
        if (isPositionLevelCall) {
            ...
            markUnsupported(`pending-order management rule ${index} (${site.functionName})`, !target.provable
                ...
                : !condition.provable
                    ...
                    : `this "${site.functionName}" call's new price could not be resolved without first knowing the target order's own type (LIMIT vs STOP) — only a call guarded by a provable order-type filter can be compiled safely`, "WARNING", "this rule is recorded in pendingOrderManagement for provenance/audit but is EXCLUDED from the compiled, executable policy (executableRules()) — never approximated, never silently executed");
        }
        return {
    ...
    markUnsupported("strategy timezone", usesLocalTime ? "source calls TimeLocal() — depends on the executing machine's local clock" : "source uses TimeCurrent()/TimeToStruct() (broker/server time), which is not a fixed IANA zone", usesLocalTime ? "BLOCKING" : "WARNING", usesLocalTime ? "results are not reproducible across machines/timezones until this is resolved" : "must be confirmed against the actual broker's server timezone before live/backtest parity can be claimed");
    // --- Repainting (Q0.8.35) ---
    ...
    const repaintingModel = isRealtimeDependent ? "REALTIME_DEPENDENT" : model.newBarDetectionSites.length > 0 ? "NON_REPAINTING" : "UNKNOWN";
    ...
    const provenanceUnsupported = [
        ...
        ...model.unsupportedConstructs
            ...
            .map((u) => ({ feature: u.functionName, reason: `"${u.functionName}" is a ${u.category} construct`, severity: "BLOCKING", executionImpact: "cannot be executed by the simulation engine", sourceLocation: { line: u.position.line, sourceHash: document.sourceHash } })),
    ];
    ...
    const semanticStatus = hasBlockingUnsupported ? "UNSUPPORTED" : approximations.length > 0 ? "APPROXIMATED" : provenanceUnsupported.length > 0 ? "SEMANTIC_EQUIVALENT" : "EXACT";
    ...
    const ir = {
        ...
        positionManagement: {
            ...
            pyramiding: gatingPositionQueries.length === 0
            ...
            reversal: gatingPositionQueries.length === 0
