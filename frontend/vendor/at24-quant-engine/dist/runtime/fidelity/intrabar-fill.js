 ...
 * semantics.
 */
export function resolveIntrabarOrderFill(order, sequence, instrument, detailTimeframe, spreadModel, slippageModel) {
    ...
    for (const obs of sequence.observations) {
        ...
        if (order.orderType === "MARKET")
            outcome = resolveMarketFill(order, childBar, spreadModel, slippageModel);
        else if (order.orderType === "LIMIT" || triggered)
            outcome = resolveLimitFill(order, childBar);
        else if (order.orderType === "STOP")
            outcome = resolveStopFill(order, childBar);
        else
            outcome = resolveStopLimitFill(order, childBar);
        if (outcome.filled) {
    ...
    return {
        ...
        reason: sequence.observations.length === 0
 ...
 */
export function resolveIntrabarProtectiveExit(side, stopLoss, takeProfit, sequence, instrument, detailTimeframe) {
