 ...
 */
function toPendingOrderManagementRule(rule) {
    return {
        id: rule.id,
        target: rule.target,
...
function blockedResult(diagnostics, unsupportedFeatures, approximations) {
    return {
        status: "BLOCKED",
        diagnostics,
        semanticLoss: {
 ...
 * except why).
 */
export function reduceStrategyIRToSpec(ir) {
    ...
    const entryRules = ir.entries.map((e) => ({
        id: e.id,
    ...
    if (!specValidation.valid) {
        ...
        return blockedResult(specValidation.errors.map((e) => `StrategySpec validation failed post-reduction: ${e}`), unsupportedFeatures, approximations);
    }
    const sourceTrace = [
