 ...
 */
const SINGLE_OUTPUT_INDICATOR_DEFS = { SMA: sma, EMA: ema, RSI: rsi, ATR: atr };
/**
 ...
 */
export function buildIndicatorSeriesFromIR(ir, bars) {
    ...
    for (const ind of ir.indicators) {
        ...
        const values = calculateSeries(def, bars, { period });
        const key = indicatorKey(indicator(ind.family, ...ind.params));
        series.set(key, values.map((v) => v ?? undefined));
        if (ind.family === "ATR")
            atrByIndex = values.map((v) => v ?? undefined);
        warmupBars = Math.max(warmupBars, def.warmup({ period }).bars);
    }
 ...
 */
export function compileToSimulation(compilation, bars, options) {
    ...
    const slicedSeries = new Map();
    for (const [key, values] of series)
        slicedSeries.set(key, values.slice(warmupBars));
