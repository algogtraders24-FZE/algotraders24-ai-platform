 ...
 */
function resolveSeriesValue(ref, bars, indexFromEnd) {
...
export function resolveOperand(operand, state) {
    ...
    if (operand.kind === "series")
        return resolveSeriesValue(operand.ref, state.bars, operand.ref.offset);
 ...
 */
function resolvePreviousOperand(operand, state) {
    if (operand.kind === "literal")
        return operand.value;
    if (operand.kind === "series") {
 ...
 */
function evaluateCross(expr, state) {
    const currentLeft = resolveOperand(expr.left, state);
    const currentRight = resolveOperand(expr.right, state);
    const previousLeft = resolvePreviousOperand(expr.left, state);
    const previousRight = resolvePreviousOperand(expr.right, state);
    if (previousLeft === undefined || previousRight === undefined)
        return false;
        ...
        case "logical": {
            switch (expr.operator) {
                ...
                case "NOT": {
                    ...
                    if (only === undefined)
                        throw new Error("NOT expression has no operand");
