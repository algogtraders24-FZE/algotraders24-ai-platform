...
export class NowPaymentsProvider {
  ...
  // Marketplace one-time purchase invoice - order_id is `mkt_<intentId>`
  // rather than encoding buyer/listing/release fields directly (unlike the
  // subscription path above), since there are too many fields to pack
  // safely into order_id; the webhook looks the context up from
  // MarketplacePurchaseIntent instead. Success/cancel land back on the
  // listing page, not /dashboard/billing.
  async createMarketplaceInvoice(params: {
    intentId: string;
    priceUsd: number;
    listingTitle: string;
    listingSlug: string;
    ...
    try {
      res = await fetch(`${API_BASE}/invoice`, {
        ...
        body: JSON.stringify({
          price_amount: params.priceUsd,
          price_currency: "usd",
          order_id: `mkt_${params.intentId}`,
          order_description: `Marketplace purchase: ${params.listingTitle}`,
          ipn_callback_url: `${siteUrl}/api/webhooks/nowpayments`,
          success_url: `${siteUrl}/marketplace/${params.listingSlug}?checkout=success`,
