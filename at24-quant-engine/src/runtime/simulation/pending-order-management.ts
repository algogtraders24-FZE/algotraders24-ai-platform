...
import type { PendingOrderManagementPolicy, PendingOrderManagementRule } from "../../domain/pending-order-management-policy.js";
 ...
 */
function resolveDistanceOffset(spec: DistanceSpec, referencePrice: number, atrValue: number | undefined): number | undefined {
  switch (spec.mode) {
    case "absolute":
      return spec.value;
    case "percentage":
...
function conditionHolds(rule: PendingOrderManagementRule, order: SimulationOrder, bar: OHLCVBar, atrValue: number | undefined): boolean {
  ...
  switch (c.kind) {
    ...
    case "FAVORABLE_DISTANCE": {
      ...
      if (referencePrice === undefined) return false;
...
function targetMatches(rule: PendingOrderManagementRule, order: SimulationOrder): boolean {
  const t = rule.target;
  if (t.orderTypeFilter && t.orderTypeFilter !== order.orderType) return false;
  if (t.sideFilter && t.sideFilter !== order.side) return false;
  return true;
}
 ...
 * silently).
 */
export function evaluatePendingOrderManagementPolicy(policy: PendingOrderManagementPolicy, order: SimulationOrder, bar: OHLCVBar, reason: string, atrValue?: number): OrderModificationIntent | undefined {
  for (const rule of policy.rules) {
    if (!rule.target.provable || !rule.condition.provable || rule.operation.kind === "UNKNOWN") continue;
    if (!targetMatches(rule, order)) continue;
    if (!conditionHolds(rule, order, bar, atrValue)) continue;
    ...
    switch (rule.operation.kind) {
      ...
      case "MODIFY_STOP": {
        if (order.orderType !== "STOP" && order.orderType !== "STOP_LIMIT") continue;
      ...
      case "MODIFY_LIMIT": {
        if (order.orderType !== "LIMIT" && order.orderType !== "STOP_LIMIT") continue;
        const offset = resolveDistanceOffset(rule.operation.newDistanceFromClose, bar.close, atrValue);
        if (offset === undefined) continue;
