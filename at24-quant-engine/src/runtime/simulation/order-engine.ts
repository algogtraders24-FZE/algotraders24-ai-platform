 ...
 */
export function createOrder(input: CreateOrderInput, sequence: number): SimulationOrder {
 ...
 */
export function isOrderExpired(order: SimulationOrder, context: { readonly asOf: number; readonly currentBarIndex: number; readonly creationBarIndex: number; readonly dayBoundaryOffsetMinutes?: number }): boolean {
  const policy = order.expiration;
  if (policy === undefined || policy.kind === "GTC") return false;
  switch (policy.kind) {
    case "DAY":
 ...
 */
export function transitionOrder(order: SimulationOrder, next: OrderStatus, updates: TransitionUpdates = {}): SimulationOrder {
