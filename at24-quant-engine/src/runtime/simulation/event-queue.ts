...
export class EventQueue {
  ...
  enqueue<TPayload>(input: EnqueueInput<TPayload>): SimulationEvent<TPayload> {
    ...
    this.heap.push(event as SimulationEvent);
  ...
  private static compare(a: SimulationEvent, b: SimulationEvent): number {
  ...
  private siftUp(index: number): void {
    ...
    while (i > 0) {
      const parent = (i - 1) >> 1;
      if (EventQueue.compare(this.heap[i]!, this.heap[parent]!) >= 0) break;
  ...
  private siftDown(index: number): void {
    ...
    for (;;) {
      ...
      const right = 2 * i + 2;
      let smallest = i;
      if (left < n && EventQueue.compare(this.heap[left]!, this.heap[smallest]!) < 0) smallest = left;
      if (right < n && EventQueue.compare(this.heap[right]!, this.heap[smallest]!) < 0) smallest = right;
      if (smallest === i) break;
