...
export type RiskActionMapping =
  | { readonly kind: "CREATE_ENTRY_ORDER"; readonly orderType: SimulationOrderType; readonly limitPrice?: number; readonly stopPrice?: number }
  | { readonly kind: "NO_OP" }
  | { readonly kind: "MODIFY_STOP"; readonly newStopPrice: number }
  | { readonly kind: "REDUCE_POSITION"; readonly closePercent: number }
  | { readonly kind: "FORCE_EXIT" };

export function mapRiskAction(action: RiskAction): RiskActionMapping {
  switch (action.type) {
    case "ALLOW_ENTRY":
      return {
