 ...
 */
export function resolveStopLossPrice(rule: StopLossRule | undefined, direction: Direction, entryPrice: number, atrValue: number | undefined): number | undefined {
  ...
  switch (rule.type) {
    ...
    case "atr-multiple": {
      if (atrValue === undefined) throw new Error("resolveStopLossPrice: atr-multiple rule requires an ATR value, none was supplied");
 ...
 * fully resolved.
 */
export function resolvePositionSize(sizing: PositionSizingMethod, params: SizingParams): number {
  switch (sizing.method) {
    ...
    case "percent-equity-risk": {
      ...
      if (!(riskDistance > 0)) throw new Error("resolvePositionSize: percent-equity-risk sizing requires a positive risk distance");
      const riskAmount = (sizing.percent / 100) * params.equity;
      return riskAmount / riskDistance;
    }
    case "atr-based":
      throw new Error(
        "resolvePositionSize: atr-based sizing has no resolved quantity formula in Q0.5 (docs/Q0.5_EXECUTION_MODEL.md) — use fixed-quantity, fixed-lot, or percent-equity-risk",
      );
  }
}
