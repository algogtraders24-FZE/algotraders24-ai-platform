 ...
 */
export function tryComputeR(direction: Direction, entryPrice: number, stopLoss: number, price: number): number | null {
  const riskDistance = computeRiskDistance(direction, entryPrice, stopLoss);
  if (!(riskDistance > 0)) return null;
  return computeRMultiple(riskDistance, favorableMove(direction, entryPrice, price));
}
