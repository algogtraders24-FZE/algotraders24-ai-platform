...
import type { RiskSpecification, SessionHoursRule, SessionWindow } from "../../domain/risk-specification.js";
 ...
 */
function localHourMinute(timestampMs: number, timezone: string): { hour: number; minute: number } {
...
function isWithinWindow(hour: number, minute: number, window: SessionWindow): boolean {
...
export function isWithinAnySessionWindow(timestampMs: number, session: SessionHoursRule): boolean {
 ...
 */
export function evaluateSessionEligibility(spec: RiskSpecification, asOf: number): RiskConstraintResult {
  const session = spec.sessionHours;
  if (session === undefined) return { passed: true };
  ...
  try {
    ...
    return {
      ...
      violation: makeViolation(
        "SESSION_RESTRICTION",
        "BLOCKING",
        `invalid timezone "${session.timezone}"`,
        session.timezone,
        null,
  ...
  if (!inWindow) {
    return {
      ...
      violation: makeViolation(
        "SESSION_RESTRICTION",
        "BLOCKING",
        `timestamp ${asOf} is outside all configured session windows (timezone ${session.timezone})`,
        asOf,
        null,
