...
export function reduceStrategyIRToSpec(ir: StrategyIR): ReductionResult {
  ...
  // ExitRule by design.
  const exitRules: StrategySpec["exitRules"] = ir.exits.filter((e) => e.kind === "SIGNAL_EXIT" && e.condition).map((e) => ({ id: e.id, condition: e.condition!, ...(e.appliesTo !== undefined ? { appliesTo: e.appliesTo } : {}) }));
