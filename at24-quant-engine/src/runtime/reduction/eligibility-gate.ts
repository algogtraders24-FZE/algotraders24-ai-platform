 ...
 */
export function checkReductionEligibility(ir: StrategyIR): EligibilityCheck {
  ...
  for (const entry of ir.entries) {
    ...
    if (entry.executionType === "LIMIT" || entry.executionType === "STOP_LIMIT") {
      if (entry.limitPrice === undefined) reasons.push(`entry "${entry.id}" executionType "${entry.executionType}" requires limitPrice, none is present`);
      else if (entry.limitPrice.kind === "UNSUPPORTED") reasons.push(`entry "${entry.id}" limitPrice references "${entry.limitPrice.reason}", which is not deterministically computable in this simulation model`);
    }
    if (entry.executionType === "STOP" || entry.executionType === "STOP_LIMIT") {
      if (entry.stopPrice === undefined) reasons.push(`entry "${entry.id}" executionType "${entry.executionType}" requires stopPrice, none is present`);
      else if (entry.stopPrice.kind === "UNSUPPORTED") reasons.push(`entry "${entry.id}" stopPrice references "${entry.stopPrice.reason}", which is not deterministically computable in this simulation model`);
    }
  }
  ...
  const atrPeriods = new Set<number>();
  const collectAtrPeriod = (spec: DistanceSpec | undefined) => {
    if (spec?.mode === "atr-multiple") atrPeriods.add(spec.atrPeriod);
  };
  if (ir.risk.stopLoss?.type === "atr-multiple") atrPeriods.add(ir.risk.stopLoss.atrPeriod);
