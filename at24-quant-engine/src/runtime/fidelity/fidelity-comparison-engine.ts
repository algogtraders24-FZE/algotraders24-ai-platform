...
import type { FidelityComparison, FidelityDifferenceClassification, TradeMatch } from "../../domain/fidelity/fidelity-comparison.js";
 ...
 */
export function compareFidelities(baseline: MultiFidelitySimulationResult, compared: MultiFidelitySimulationResult): FidelityComparison {
  ...
  for (let i = 0; i < matchCount; i++) {
    ...
    if (c.entryPrice !== b.entryPrice || c.exitPrice !== b.exitPrice) priceDiffers = true;
    if (c.entryTimestamp !== b.entryTimestamp || c.exitTimestamp !== b.exitTimestamp) timingDiffers = true;
  ...
  let differenceClassification: FidelityDifferenceClassification;
  if (structural) differenceClassification = "STRUCTURAL";
  else if (priceDiffers && timingDiffers) differenceClassification = "PRICE_AND_TIMING";
  else if (priceDiffers) differenceClassification = "PRICE_ONLY";
  else if (timingDiffers) differenceClassification = "TIMING_ONLY";
  else differenceClassification = "IDENTICAL"; // trades identical but some other field (e.g. provenance) differed
