...
import type { EventStatistics, SimulationExecutionStatistics } from "../../domain/simulation/simulation-result.js";
...
import type { MultiFidelityProvenance } from "../../domain/fidelity/fidelity-provenance.js";
import type { MultiFidelitySimulationResult } from "../../domain/fidelity/fidelity-simulation-result.js";
import type { DetailCoverage } from "../../domain/fidelity/detail-coverage.js";
import type { FidelityQuality } from "../../domain/fidelity/fidelity-quality.js";
...
import { resolveMarketFill, resolveLimitFill, resolveStopFill, resolveStopLimitFill, resolveProtectiveExit, type BarFillOutcome } from "../simulation/bar-fill-model.js";
 ...
 */
function runFidelityAwareSimulation(bars: readonly OHLCVBar[], config: MultiFidelityConfig): MultiFidelitySimulationResult {
  ...
  function applyRealizedTrade(state: SimulationState, position: Position, exitPrice: number, exitQuantity: number, grossPnl: number, fee: number, timestamp: number): SimulationState {
  ...
  while (event !== undefined) {
    ...
    let childBars: readonly OHLCVBar[];
    if (sequence.coverage === "MISSING") {
      ...
      if (sequence.coverage === "COMPLETE") completeParents += 1;
      else partialParents += 1;
    ...
    if (base.pendingOrderManagementPolicy) {
      ...
      if (executable.length > 0) {
        ...
        for (const [orderId, order] of state.pendingOrders) {
          ...
          if (order.creationTimestamp >= bar.timestamp) continue;
    ...
    for (const childBar of childBars) {
      ...
      for (const [orderId, order] of state.pendingOrders) {
        if (order.instrument.symbol !== symbol) continue;
        if (order.creationTimestamp >= childBar.timestamp) continue; // same-bar safety guard (parent-level, preserved at child granularity)
        ...
        if (order.orderType === "MARKET") outcome = resolveMarketFill(order, childBar, base.spreadModel, base.slippageModel);
        else if (order.orderType === "LIMIT") outcome = resolveLimitFill(order, childBar);
        else if (order.orderType === "STOP") outcome = resolveStopFill(order, childBar);
        else outcome = order.status === "TRIGGERED" ? resolveLimitFill(order, childBar) : resolveStopLimitFill(order, childBar);
      ...
      if (positionForProtectiveCheck) {
        ...
        if (exit.exited) {
          if (exit.ambiguous) ambiguousResolutionCount += 1;
  ...
  const eventStatistics: EventStatistics = { totalEvents: Object.values(eventCounts).reduce((a, b) => a + b, 0), eventsByType: { ...eventCounts } };
  ...
  const fidelityQuality: FidelityQuality = {
  ...
  const provenance: MultiFidelityProvenance = {
    strategyHash,
    strategyVersion: base.strategySpec.version,
  ...
  const metrics: Partial<Record<CoreMetricName, number>> & { averageR: number | null; totalFees: number } = { ...coreMetrics, averageR, totalFees };
 ...
 * engine above.
 */
export function runMultiFidelitySimulation(bars: readonly OHLCVBar[], config: MultiFidelityConfig): MultiFidelitySimulationResult {
  if (config.fidelity === "D1_OHLC") {
    const baseResult = runSimulation(bars, config.base);
    const detailCoverage: DetailCoverage = { totalParents: bars.length, completeParents: bars.length, partialParents: 0, missingParents: 0, completeRatio: 1 };
    const fidelityQuality: FidelityQuality = { requestedFidelity: "D1_OHLC", detailCoverage, ambiguousResolutionCount: 0, parentsResolvedAtParentGranularity: 0 };
    const provenance: MultiFidelityProvenance = {
