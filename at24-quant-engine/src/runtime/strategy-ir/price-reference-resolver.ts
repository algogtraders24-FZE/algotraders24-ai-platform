 ...
 */
export function resolvePriceReference(ref: PriceReference, state: MarketState): number {
  switch (ref.kind) {
    case "OPERAND": {
    ...
    case "MID": {
      ...
      if (!bar) throw new Error("resolvePriceReference: MID requires at least one bar of history");
