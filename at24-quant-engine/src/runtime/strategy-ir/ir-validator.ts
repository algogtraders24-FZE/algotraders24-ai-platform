...
function declaredIndicatorKeys(indicators: readonly IndicatorIR[]): Set<string> {
  const keys = new Set<string>();
  for (const ind of indicators) {
    if (ind.kind === "named") keys.add(`${ind.family}(${ind.params.join(",")})`);
    else keys.add(`${ind.name}(${ind.parameters.join(",")})`);
  }
  return keys;
}

function collectIndicatorRefs(expr: Expression, out: Set<string>): void {
  switch (expr.type) {
    case "comparison":
      for (const operand of [expr.left, expr.right]) {
        if (operand.kind === "indicator") out.add(indicatorKey(operand.ref));
      }
      return;
 ...
 */
function checkIndicatorReferencesDeclared(ir: StrategyIR): readonly string[] {
  const declared = declaredIndicatorKeys(ir.indicators);
  const used = new Set<string>();
  ir.entries.forEach((e) => {
    collectIndicatorRefs(e.condition, used);
    if (e.trigger) collectIndicatorRefs(e.trigger, used);
  });
  ir.exits.forEach((e) => {
    if (e.condition) collectIndicatorRefs(e.condition, used);
  });
 ...
 */
function checkHTFLookahead(ir: StrategyIR): readonly string[] {
...
function checkRepainting(ir: StrategyIR): readonly string[] {
  if (ir.repaintingModel === "REPAINTING") return [`repaintingModel is "REPAINTING" — this strategy's values change retroactively and cannot be safely executed (Q0.7.22)`];
  if (ir.repaintingModel === "UNKNOWN") return [`repaintingModel is "UNKNOWN" — repainting risk must be resolved (NON_REPAINTING/CONFIRMED_ONLY/REALTIME_DEPENDENT) before execution eligibility can be granted (Q0.7.22)`];
  return [];
}
...
function checkUnsupportedSemantics(ir: StrategyIR): readonly string[] {
 ...
 */
export function validateStrategyIR(ir: StrategyIR): IRValidationResult {
