...
export type SlippageAssumption =
  | { readonly type: "fixed-points"; readonly value: number }
  | { readonly type: "percent"; readonly value: number };

export type SpreadAssumption = { readonly type: "fixed-points"; readonly value: number };

/** A flat per-trade fee, distinct from the existing per-unit `commissionPerUnit`. */
export type FeeAssumption = { readonly type: "fixed-per-trade"; readonly value: number };

export type LatencyAssumption = { readonly type: "fixed-ms"; readonly value: number };

/** Which price a fill is measured against. */
export type PriceBasis = "bid" | "ask" | "mid";

export type MarginAssumption = { readonly leverage: number };
...
export function validateExecutionSpecification(spec: ExecutionSpecification): ValidationResult {
  ...
  if (costsUnset && spec.costsExplicitlyZero !== true) {
    results.push(
      fail(
        "execution costs (spread/slippage/commissionPerUnit/fee) are all unset — " +
